...
if(isset($_GET['item'])&&isset($_GET['key']))
{
	...
	xtc_session_register('xtb0');
	xtc_session_register('xtb1');
	
	# - herausfinden um welchen ebay user es sich handelt
	# - transaktionsids des users finden und loopen, um warenkorb zu fuellen
	
	$q = xtc_db_query("SELECT * FROM xtb_transactions as t LEFT JOIN xtb_auctions as a ON (a._EBAY_ITEM_ID=t.XTB_ITEM_ID) WHERE t.XTB_KEY = '".xtc_db_input($_GET['key'])."' AND MD5(CONCAT(t.XTB_ITEM_ID,'',t.XTB_EBAY_USERID)) = '".xtc_db_input($_GET['item'])."' AND t.XTB_CHECKOUT_TS=0");
...
$sql = "SELECT * FROM xtb_auctions WHERE _EBAY_ITEM_ID = '".xtc_db_input($_GET['EID'])."' AND _XTB_ITEM_HASH='".xtc_db_input($_GET['ITEM_HASH'])."' AND _EBAY_QUANTITY_BUYED < QUANTITY";
...
if(xtc_db_num_rows($products_query)==1)
{
	...
	#
	$product_data_query = xtc_db_query("SELECT * FROM " . TABLE_PRODUCTS . " p, " . TABLE_LANGUAGES . " l, " . TABLE_PRODUCTS_DESCRIPTION . " pd left join ".TABLE_PRODUCTS_IMAGES." as pi ON (pi.products_id = pd.products_id) WHERE p.products_id = pd.products_id AND pd.language_id = l.languages_id AND l.code = '" . xtc_db_input($_GET['TEMPLATES_LANGUAGE']). "' AND p.products_id = '".$x['products_id']."'");
