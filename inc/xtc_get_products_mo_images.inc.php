<?php
/* --------------------------------------------------------------
   xtc_get_products_mo_images.inc.php 2010-05-06 gm
   Gambio GmbH
   http://www.gambio.de
...
/* ----------------------------------------------------------------------------------------- 
   ...
   function xtc_get_products_mo_images($products_id = '', $p_ignore_gm_show_image = false){
	...
	{
		$mo_query = "select image_id, image_nr, image_name from " . TABLE_PRODUCTS_IMAGES . " where products_id = '" . (int)$products_id ."' ORDER BY image_nr";
	}
   	else
   	{
