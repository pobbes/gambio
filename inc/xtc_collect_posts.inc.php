<?php
/* --------------------------------------------------------------
   xtc_collect_posts.php 2012-04-17 gm
   Gambio GmbH
   http://www.gambio.de
   ...
   (c) 2003 XT-Commerce - community made shopping http://www.xt-commerce.com ($Id: xtc_collect_posts.inc.php 803 2005-02-26 15:32:50Z mz $)
   ...
   ---------------------------------------------------------------------------------------*/
    ...
    function xtc_collect_posts() {
      ...
      if ($_POST['gv_redeem_code']) {
        $gv_query = xtc_db_query("select coupon_id, coupon_amount, coupon_type, coupon_minimum_order,uses_per_coupon, uses_per_user, restrict_to_products,restrict_to_categories from " . TABLE_COUPONS . " where coupon_code='".xtc_db_input($_POST['gv_redeem_code'])."' and coupon_active='Y'");
...
//        xtc_redirect(xtc_href_link(FILENAME_SHOPPING_CART, 'info_message=' . urlencode(REDEEMED_AMOUNT. $xtPrice->xtcFormat($gv_amount,true,0,true)), 'SSL'));
        $_SESSION['info_message'] = REDEEMED_AMOUNT. $xtPrice->xtcFormat($gv_amount,true,0,true);
        xtc_redirect(xtc_href_link(FILENAME_SHOPPING_CART, '', 'SSL'));
...
//        	xtc_redirect(xtc_href_link(FILENAME_SHOPPING_CART, 'info_message=' . urlencode(ERROR_NO_INVALID_REDEEM_COUPON), 'SSL'));
        	$_SESSION['info_message'] = ERROR_NO_INVALID_REDEEM_COUPON;
        	xtc_redirect(xtc_href_link(FILENAME_SHOPPING_CART, '', 'SSL'));
        }
...
//        	xtc_redirect(xtc_href_link(FILENAME_SHOPPING_CART, 'info_message=' . urlencode(ERROR_INVALID_STARTDATE_COUPON), 'SSL'));
        	$_SESSION['info_message'] = ERROR_INVALID_STARTDATE_COUPON;
        	xtc_redirect(xtc_href_link(FILENAME_SHOPPING_CART, '', 'SSL'));
        }
...
//     		xtc_redirect(xtc_href_link(FILENAME_SHOPPING_CART, 'info_message=' . urlencode(ERROR_INVALID_FINISDATE_COUPON), 'SSL'));
       		$_SESSION['info_message'] = ERROR_INVALID_FINISDATE_COUPON;
       		xtc_redirect(xtc_href_link(FILENAME_SHOPPING_CART, '', 'SSL'));
        }
...
//        	xtc_redirect(xtc_href_link(FILENAME_SHOPPING_CART, 'info_message=' . urlencode(ERROR_INVALID_USES_COUPON . $gv_result['uses_per_coupon'] . TIMES ), 'SSL'));
        	$_SESSION['info_message'] = ERROR_INVALID_USES_COUPON . $gv_result['uses_per_coupon'] . TIMES;
        	xtc_redirect(xtc_href_link(FILENAME_SHOPPING_CART, '', 'SSL'));
        }
...
//        	xtc_redirect(xtc_href_link(FILENAME_SHOPPING_CART, 'info_message=' . urlencode(ERROR_INVALID_USES_USER_COUPON . $gv_result['uses_per_user'] . TIMES ), 'SSL'));
        	$_SESSION['info_message'] = ERROR_INVALID_USES_USER_COUPON . $gv_result['uses_per_user'] . TIMES;
        	xtc_redirect(xtc_href_link(FILENAME_SHOPPING_CART, '', 'SSL'));
        }
...
//        xtc_redirect(xtc_href_link(FILENAME_SHOPPING_CART, 'info_message=' . urlencode(REDEEMED_COUPON), 'SSL'));
        $_SESSION['info_message'] = REDEEMED_COUPON;
        xtc_redirect(xtc_href_link(FILENAME_SHOPPING_CART, '', 'SSL'));
    }
...
//     	xtc_redirect(xtc_href_link(FILENAME_SHOPPING_CART, 'info_message=' . urlencode(ERROR_NO_REDEEM_CODE), 'SSL'));
     	$_SESSION['info_message'] = ERROR_NO_REDEEM_CODE;
     	xtc_redirect(xtc_href_link(FILENAME_SHOPPING_CART, '', 'SSL'));
     }
   }
