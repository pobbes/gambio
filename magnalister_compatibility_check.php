 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: magnalister_compatibility_check.php 140 2011-11-30 01:52:39Z derpapst $
 *
...
$strings = array (
	'en' => array (
		...
		'configure_not_found' => 'The file <span class="tt">%s</span> could not be found. Is the script positioned in the proper directory?',
	...
	'de' => array (
		...
		'configure_not_found' => 'Die Datei <span class="tt">%s</span> konnte nicht gefunden werden. Liegt das Script im richtigen Verzeichnis?',
...
if (file_exists('includes/configure.php')) {
	...
	$mySQLVersion = mysql_fetch_array(mysql_query('SELECT VERSION()'), MYSQL_NUM);
	$mySQLVersion = $mySQLVersion[0];
	mysql_close();
} else {
	$messages[] = '<p>'.sprintf(LANG_CONFIGURE_NOT_FOUND, 'includes/configure.php').'</p>';
}
...
if (($localClientVersion = fileGetContents($currentClientURL, $warnings, -1, $method)) === false) {
...
if (($localClientVersionPHP = fileGetContentsPHP($currentClientURL, $filePHPError, -1, $phpmethod)) === false) {
...
/* cURL Check */
$localClientVersionCURL = fileGetContentsCURL($currentClientURL, false, $fileCURLError, -1, $curlmethod);
if ($localClientVersionCURL === false) {
	$localClientVersionCURL = fileGetContentsCURL($currentClientURL, true, $fileCURLError, -1, $curlmethod);
}
if ($localClientVersionCURL !== false) {
		...
		<table id="systemcheck">
			<tbody>
				...
				<tr>
					...
					'.(empty($localClientVersion) ? '<td class="fail">X</td>' : '<td class="ok">OK</td>').'<td>&mdash;</td>
				</tr>
				<tr class="space">
					<td colspan="3">&nbsp;</td>
				</tr>
				...
				<tr>
					...
					'.(empty($localClientVersionCURL) ? '<td class="fail">X<div class="why">?<div class="reason">'.$fileCURLError.'</div></div></td>' : '<td class="ok">OK</td>').'<td>&mdash;</td>
				</tr>' : '').'
				<tr>
