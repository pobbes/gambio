<?php
##
## xs:booster v1.0424 für GambioGX
...
elseif($xtb_module=='add_ajx') {
	...
	else
	{
		...
		if($x['products_vpe_status']==1) {
			if($x['products_vpe_value']==0) $x['products_vpe_value'] = 1;
			if ($x['products_tax_class_id'] == 2) { $ms_price = round($x['products_price']*(7+100)/100, 2); }
			else { $ms_price = round ($x['products_price']*(19+100)/100, 2); }
			$desc = str_replace("#ARTICLE_VPE#", $xtPrice->xtcFormat($ms_price * (1.0 / $x['products_vpe_value']), true)." / ".xtc_get_vpe_name($x['products_vpe']), $desc);
                    }
                    else {
                       $desc = str_replace("#ARTICLE_VPE#", "", $desc);
                    }
		}
	...
	if(preg_match("/#ARTICLE_PRICE#/",$desc))
		$item['DESCRIPTION'] = str_replace("#ARTICLE_PRICE#", $xtPrice->xtcFormat($item['STARTPRICE'],true), $item['DESCRIPTION']);
	...
	$title = utf8_substr(strip_tags($item['TITLE']),0,54);
...
<script type="text/javascript"> if(window.jQuery !== undefined){ jQuery.noConflict(); } </script>
...
<!-- header_eof //-->
<script type="text/javascript">  if(isdefined("jQuery")) { jQuery.noConflict(); } </script>
...
TEMPLATE: -=".base64_encode($_POST['content_manager'])."
...
<form name="config_form" method="post" action="xtbooster.php" enctype="multipart/form-data">
...
<input type="text" name="MODULE_XTBOOSTER_SHOPKEY" value="<?php echo $xtb_config['MODULE_XTBOOSTER_SHOPKEY'] ?>" size="32" maxlength="32"><?php if($xtb_config['MODULE_XTBOOSTER_SHOPKEY']=='') echo TXT_SHOPKEY_DESCR?>&nbsp;</td>
	</tr>
...
<?php } # ende if(xtc_db_num_rows > 1)
  else # descriptions nur in 1 Sprache vorhanden
...
{
   $emailFCKeditor = new FCKeditor('MODULE_XTBOOSTER_EMAILTEMPLATE_HTML');
   $emailFCKeditor->BasePath   = DIR_WS_ADMIN . 'gm/fckeditor/';
   $emailFCKeditor->Height = 320;
   $emailFCKeditor->Width = 550;
   $emailFCKeditor->Value = $emailtemplate_html;
   $emailFCKeditor->ToolbarSet = "Big";
   $emailFCKeditor->Config["DefaultLanguage"] = $_SESSION['language_code'];
   $emailFCKeditor->Create();
}
endif;
?>
                                    </div>
...
<script type="text/javascript">
  ...
  $('DIV_TXT_MAIL').style.display='<?php echo $txt_mail_display ?>'; 
		...
		<?php endif; ?>
		<?php if(intval($extra_features['PicturesInHtmlMail'] != 0))
...
if($xtb_config['MODULE_XTBOOSTER_SHOPKEY']!='')
{
   $oFCKeditor = new FCKeditor('content_manager');
   $oFCKeditor->BasePath   = DIR_WS_ADMIN . 'gm/fckeditor/';
   $oFCKeditor->Height = 400;
   $oFCKeditor->Width = 550;
   $oFCKeditor->Value = $tradetemplate;
...
<?php endif; # end of if($xtb_config['MODULE_XTBOOSTER_SHOPKEY']!=''):?>
</table>
...
<tr class="attributes<?php echo $rowi++%2==0?'-even':'-odd'; ?>">
	<td class="smallText" style="width:200px;font-weight:bold;" valign="top">ShopKey</td>
	<td class="smallText"><?php echo $xtb_config['MODULE_XTBOOSTER_SHOPKEY'] ?></td>
</tr>
<?php if(!$multi_xtb): ?>
<tr class="attributes<?php echo $rowi++%2==0?'-even':'-odd'; ?>">
...
<tr class="attributes<?php echo $rowi++%2==0?'-even':'-odd'; ?>">
	...
	<td class="smallText"><input type="text" name="add[TITLE]" value="<?php echo htmlspecialchars(strip_tags($x['products_name']))?>" size="30" maxlength="80"></td>
</tr>
<tr class="attributes<?php echo $rowi++%2==0?'-even':'-odd'; ?>">
...
if($xtb_config['MODULE_XTBOOSTER_SHOPKEY']!='')
{
   $oFCKeditor = new FCKeditor('add[DESCRIPTION]');
   $oFCKeditor->BasePath   = DIR_WS_ADMIN . 'gm/fckeditor/';
   $oFCKeditor->Height = 400;
   $oFCKeditor->Width = 550;
...
switch($filter)
{
	...
	case 0:		$add = "(_EBAY_STATUS='active' AND _EBAY_QUANTITY_BUYED=0)"; break;
	# aktiv, erfolgreich 
	case 1:		$add = "(_EBAY_STATUS='active' AND _EBAY_QUANTITY_BUYED>0)"; break;
	# beendet, erfolgreich 
	case 2:		$add = "(_EBAY_STATUS='successful')"; break;
	# beendet, erfolglos
	case 3:		$add = "(_EBAY_STATUS='unsuccessful')"; break;
	# eBay-Verkäufe chronologisch
	case 4:         $add = "(_EBAY_QUANTITY_BUYED>0)"; break;
}
...
while($x = xtc_db_fetch_array($products_query)) # This line includes GNU/GPL licensed code written by xt:Commerce GmbH (www.xtcommerce.de)
{
	...
	switch($x['TYPE'])
	{
		case 'FixedPriceItem': $x['TYPE']=TXT_FIXPRICEAUCTION; break;
		case 'Chinese': $x['TYPE']=TXT_CHINESEAUCTION; break;
		case 'Dutch': $x['TYPE']=TXT_DUTCHAUCTION; break;
		case 'StoresFixedPrice': $x['TYPE']=TXT_STOREFIXEDPRICE; break;
	}
