...
/* --------------------------------------------------------------
  ...
  switch ($_GET['action']) {
    case 'insert':
      ...
      // create additional gm_contents records
	        ...
	        while ($gm_contents = xtc_db_fetch_array($gm_contents_query)) {
	          xtc_db_query("insert into gm_contents (languages_id,
	                                          gm_key,
	                                          gm_value,
	                                          gm_group_id,
	                                          gm_sort_order
	                                         ) values ('" . $insert_id . "', '" . $gm_contents['gm_key'] . "', '" . $gm_contents['gm_value'] . "','".$gm_contents['gm_group_id']."','".$gm_contents['gm_sort_order']."')");
      }


    // create additional gm_emails records
	        $gm_emails_query = xtc_db_query("SELECT
	                                          gm_email_id,
	                                          folder,
	                                          filename,
	                                          ...
	                                          FROM gm_emails
	                                          WHERE languages_id='".$_SESSION['languages_id']."'");
	        while ($gm_emails = xtc_db_fetch_array($gm_emails_query)) {
	          xtc_db_query("insert into gm_emails (gm_email_id,
	                                          languages_id,
	                                          folder,
	                                          filename,
	                                          backup_original_html,
											  ...
											  backup_original_txt,
	                                          backup_user_txt
	                                         ) values ('" . $gm_emails['gm_email_id'] . "', '" . $insert_id . "', '" . $gm_emails['folder'] . "','".$gm_emails['filename']."','". addslashes(str_replace($_SESSION['language'].'/lang_'.$_SESSION['language'],$directory.'/lang_'.$directory,$gm_emails['backup_original_html']))."','".addslashes(str_replace($_SESSION['language'].'/lang_'.$_SESSION['language'],$directory.'/lang_'.$directory,$gm_emails['backup_user_html']))."','".addslashes(str_replace($_SESSION['language'].'/lang_'.$_SESSION['language'],$directory.'/lang_'.$directory,$gm_emails['backup_original_txt']))."','".$gm_emails['gm_sort_order']."')");
      }
    ...
    case 'deleteconfirm':
      ...
      xtc_db_query("delete from gm_emails where languages_id = '" . xtc_db_input($lID) . "'");
...
<!-- body_text //-->
    <td class="boxCenter" width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td width="100%">
		<div class="pageHeading" style="background-image:url(images/gm_icons/land.png)"><?php echo HEADING_TITLE; ?></div>
		<br />
		<?php echo HEADING_WARNING; ?>
		</td>
      </tr>
      <tr>
        <td>
	...
	<span class="main">
		<table style="margin-bottom:5px" border="0" cellpadding="0" cellspacing="0" width="100%">
			<tr class="dataTableHeadingRow">
				<td class="dataTableHeadingContentText" style="border-right: 0px"><?php echo GM_LANGUAGE_CONFIGURATION_TITLE; ?></td>
			</tr>
		</table>
		...
		<table style="border: 1px solid #dddddd" border="0" cellpadding="0" cellspacing="0" width="100%">
			<tr class="dataTableRow">
				<td style="font-size: 12px; padding: 0px 10px 11px 10px; text-align: justify">
					...
					if(isset($_POST['gm_save']))
					{
						echo '<div id="gm_language_configuration_success" style="background-color: #408E2F; color: #ffffff; font-weight: bold; padding: 5px; margin-top: 10px">' . GM_LANGUAGE_CONFIGURATION_SUCCESS . '</div>';
						echo '<script type="text/javascript">setTimeout("$(\'#gm_language_configuration_success\').fadeOut(\'slow\')", 10000);</script>';
					}
					?>					
					<form action="<?php xtc_href_link('languages.php'); ?>" method="post">
						...
						<br />
						<br />					
						<input style="margin-left:1px" type="submit" name="gm_save" class="button" onClick="this.blur();" value="<?php echo BUTTON_SAVE; ?>" />
					</form>
...
<?php
  ...
  switch ($_GET['action']) {
    case 'new':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br /><div align="center"><input class="button" value="' . BUTTON_INSERT . '" type="submit" /><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_LANGUAGES, 'page=' . $_GET['page'] . '&lID=' . $_GET['lID']) . '">' . BUTTON_CANCEL . '</a></div>');
      break;
    ...
    case 'edit':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br /><div align="center"><input type="submit" class="button"  value="' . BUTTON_UPDATE . '"><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_LANGUAGES, 'page=' . $_GET['page'] . '&lID=' . $lInfo->languages_id) . '">' . BUTTON_CANCEL . '</a></div>');
      break;
