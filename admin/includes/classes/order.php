  ...
  class order {
    ...
    function query($order_id) {
      ...
      while ($orders_products = xtc_db_fetch_array($orders_products_query)) {
		...
		# properties
        $subindex = 0;
        $attributes_query = xtc_db_query("select * from orders_products_properties where orders_products_id = '" . $orders_products['orders_products_id'] . "'");
        if (xtc_db_num_rows($attributes_query)) {
          while ($attributes = xtc_db_fetch_array($attributes_query)) {
            $this->products[$index]['properties'][$subindex] = array('properties_name' => $attributes['properties_name'],
