<?php
/* --------------------------------------------------------------
   class.paypal.php 2010-01-22 gm
   Gambio GmbH
   http://www.gambio.de
		...
		if(xtc_db_num_rows($query)){
			...
			if($response['ADDRESSSTATUS'] != '') {
				$output .= '<tr>
					...
					<td class="main">(' . $response['ADDRESSSTATUS'] . ')<br>' . utf8_decode($response['SHIPTONAME']) . '<br>' . utf8_decode($response['SHIPTOSTREET']) . '<br>' . $response['SHIPTOZIP'] . ' ' . utf8_decode($response['SHIPTOCITY']) . '<br>' . utf8_decode($response['SHIPTOCOUNTRYNAME']) . '</td>
					</tr>';
			}
			$output .= '<tr>
