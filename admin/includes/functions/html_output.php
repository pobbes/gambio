<?php
/* --------------------------------------------------------------
   html_output.php 2012-09-26 gambio
   Gambio OHG
   http://www.gambio.de
...
require_once (DIR_FS_INC.'clean_param.inc.php');
  ...
  function xtc_href_link($page = '', $parameters = '', $connection = 'NONSSL') {
	$parameters = clean_param($parameters, true);
 	...
 	if(is_array($values))
   	{
         foreach ($values as $key=>$val) {
             ...
             $field .= '>' . htmlentities_wrapper(html_entity_decode($val['text'])) . '</option>';
         }
   	} 
    $field .= '</select>';
...
//GM_MOD:
          ...
          $nav.=xtc_image(DIR_WS_ICONS . 'sort_down.gif', '', '14' ,'12', 'class="sort_image"').'</a>';
          $nav.='<a href="'.xtc_href_link(FILENAME_CUSTOMERS,'sorting='.$sort.'-desc&'.xtc_get_all_get_params(array('action','sorting'))).'">';
          $nav.= xtc_image(DIR_WS_ICONS . 'sort_up.gif', '', '14' ,'12', 'class="sort_image"').'</a></div>';

          break;
          
          case FILENAME_CATEGORIES:
//GM_MOD:
          $nav='<div><a href="'.xtc_href_link(FILENAME_CATEGORIES,'sorting='.$sort.'&'.xtc_get_all_get_params(array('action','sorting'))).'">';
          $nav.=xtc_image(DIR_WS_ICONS . 'sort_down.gif', '', '14' ,'12', 'class="sort_image"').'</a>';
