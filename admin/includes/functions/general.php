...
function check_stock($products_id) {
	...
	$attribute_stock_query = xtc_db_query("SELECT DISTINCT attributes_stock, options_values_id FROM ".TABLE_PRODUCTS_ATTRIBUTES." where products_id = '".$products_id."'");
	while ($attribute_stock_values = xtc_db_fetch_array($attribute_stock_query)) {
		if ($attribute_stock_values['attributes_stock'] <= '0') {
			...
			if(!isset($stock_warn)) $stock_warn .= $which_attribute['products_options_values_name'];
			else $stock_warn .= ', '.$which_attribute['products_options_values_name'];
		}
	}
	// EOF GM_MOD
...
function xtc_remove_order($order_id, $restock = false, $canceled = false, $reshipp = false)
{	
	...
	if(!$canceled) {
		...
		$t_get_paypal_ipn_id = xtc_db_query("
											...
											FROM
												paypal
											WHERE
												xtc_order_id = '".xtc_db_input($order_id)."'");

		while($t_paypal_ipn_id = xtc_db_fetch_array($t_get_paypal_ipn_id)) {
			xtc_db_query("DELETE FROM paypal_status_history WHERE `paypal_ipn_id` = '".xtc_db_input($t_paypal_ipn_id['paypal_ipn_id'])."'");
		}
		xtc_db_query("DELETE FROM paypal WHERE `xtc_order_id` = '".xtc_db_input($order_id)."'");
		/********* PayPal ***********/
	} 
}
...
function xtc_convert_linefeeds($from, $to, $string) {
	if ((PHP_VERSION < "4.0.5") && is_array($from)) {
		return preg_replace('!('.implode('|', str_replace('!', '\!', $from)).')!', $to, $string);
	} else {
...
function xtc_get_status_users($status_id) {
	$status_query = xtc_db_query("SELECT count(*) as count FROM ".TABLE_CUSTOMERS." WHERE customers_status = '".$status_id."'");
...
function gm_cfg_select_option($select_array, $key_value, $key = '') {
	for ($i = 0, $n = sizeof($select_array); $i < $n; $i ++) {
		...
		if ($key_value == $select_array[$i])
			$string .= ' checked';
		$string .= ' /> ' . constant('GM_CFG_' . strtoupper($select_array[$i])) .'<br />';
	}
