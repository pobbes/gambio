<?php
/* --------------------------------------------------------------
   categories_view.php 2009-11-20 mb
   Gambio GmbH
   http://www.gambio.de
         ...
         <td valign="top">
            ...
            <tr class="dataTableHeadingRow">
             ...
             <td class="dataTableHeadingContent" align="center" width="7%">
                <?php echo TABLE_HEADING_SORT; ?>
             </td>
             <td class="dataTableHeadingContent" align="center" width="35%">
                <?php echo TABLE_HEADING_CATEGORIES_PRODUCTS; ?>
             </td>
             <?php
             // check Produkt and attributes stock
             if (STOCK_CHECK == 'true') {
                    echo '<td class="dataTableHeadingContent" align="center" width="15%">Lager</td>';
             }
             ?>
             <td class="dataTableHeadingContent" align="center" width="6%">
                <?php echo TABLE_HEADING_STATUS; ?>
             </td>
             <td class="dataTableHeadingContent" align="center" width="8%">
                <?php echo TABLE_HEADING_STARTPAGE; ?>
             </td>
             <td class="dataTableHeadingContent" align="center" width="10%">
                <?php echo TABLE_HEADING_PRICE; ?>
             </td>
             <td class="dataTableHeadingContent" align="center" width="10%">
                <?php echo TABLE_HEADING_MAX; ?>
             </td>
             <td class="dataTableHeadingContent" align="center" width="10%">
               &nbsp;
             </td>
            ...
            <tr class="dataTableHeadingRow_sortbar">
             ...
             <td class="dataTableHeadingContent_sortbar" align="center" width="30%">
                <?php echo xtc_sorting(FILENAME_CATEGORIES,'name'); ?>
             </td>
             <?php
             // check Produkt and attributes stock
             if (STOCK_CHECK == 'true') {
                    echo '<td class="dataTableHeadingContent_sortbar" align="center" width="20%">' . xtc_sorting(FILENAME_CATEGORIES,'stock') . '</td>';
             }
             ?>
             <td class="dataTableHeadingContent_sortbar" align="center" width="7%">
             ...
             <td class="dataTableHeadingContent_sortbar" width="10%" align="center">
                &nbsp;
             </td>
            </tr>						
...
// ----------------------------------------------------------------------------------------------------- //
    ...
    while ($products = xtc_db_fetch_array($products_query)) {
      ...
      <td class="categories_view_data" style="text-align: left; padding-left: 8px;">
        <?php echo  xtc_image(DIR_WS_ICONS . 'preview.gif', '') . '&nbsp;</a><a href="'.xtc_href_link(FILENAME_CATEGORIES, xtc_get_all_get_params(array('cPath', 'action', 'pID', 'cID')) . 'cPath=' . $cPath . '&pID=' . $products['products_id']) .'">' . $products['products_name']; ?></a>
      </td>          
      <?php
      // check product and attributes stock
      if (STOCK_CHECK == 'true') { ?>
...
?>
        ...
        <!-- bottom buttons -->
        <table border="0" width="100%" cellspacing="0" cellpadding="2" style="padding-top: 10px; border-top: 1px solid Black; margin-top:-1px">
        <tr>
         ...
         <?php
			...
			echo '<a style="white-space: nowrap;" class="button float_right" href="javascript:SwitchCheck()" onClick="this.blur()">' . BUTTON_REVERSE_SELECTION . '</a>';
            echo '<a class="button float_right" href="javascript:SwitchProducts()" onClick="this.blur()">' . BUTTON_SWITCH_PRODUCTS . '</a>';
            echo '<a class="button float_right" href="javascript:SwitchCategories()" onClick="this.blur()">' . BUTTON_SWITCH_CATEGORIES . '</a>';  
...
<?php
    ...
    /* xs:booster */
    $xsbooster_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_XTBOOSTER_STATUS'");
    if (0 < xtc_db_num_rows ($xsbooster_query)) $xsbooster_installed = true; else $xsbooster_installed = false;
    /* END xs:booster */
    switch ($_GET['action']) {        
      ...
      default:
	    ...
	    if ($xsbooster_installed) $contents[] = array('text' => '<div align="center"><input type="submit" class="button" onClick="this.blur();" name="multi_xtb" value="' . BUTTON_XTBOOSTER_MULTI . '"></div>');            
            $contents[] = array('text'  => '</form>');
			...
			if(gm_get_conf('GM_GOOGLE_SHOPPING_STATUS') == 1){
				$contents[] = array('align' => 'center', 'text' => '<div align="center"><a class="button lightbox_google_admin_categories" onClick="this.blur();" href="#" rel="' . $cInfo->categories_id . '">Google Kategorien</a></div>');
			}
            ...
            $contents[] = array('align' => 'center', 'text' => '<div align="center" style="padding:10px;overflow:hidden;">' . xtc_info_image_c($cInfo->categories_image, $cInfo->categories_name)  . '</div><div style="padding-bottom: 10px;">' . $cInfo->categories_image . '</div>');            
	    ...
	    if ($xsbooster_installed) $contents[] = array('text' => '<div align="center"><input type="submit" class="button" onClick="this.blur();" name="multi_xtb" value="' . BUTTON_XTBOOSTER_MULTI . '"></div>');
            ...
            // BOF GM_MOD
						...
						if(xtc_db_num_rows($gm_check_special) == 0){
							...
							if ($xsbooster_installed) $contents[] = array('text' => '<div align="center"><form action="' . FILENAME_XTBOOSTER . '" name="edit_xtbooster" method="POST"><input type="hidden" name="action" value="edit_xtbooster"><input type="hidden" name="xtb_module" value="add"><input type="hidden" name="current_product_id" value="' . $pInfo->products_id . '"><input type="hidden" name="cpath" value="' . $cPath  . '"><input type="submit" class="button" onClick="this.blur();" value="' . BUTTON_EDIT_XTBOOSTER . '"></form></div>');
						}
						else{
            ...
            //Informations
            $contents[] = array('align' => 'center', 'text' => '<div style="padding-top: 5px; font-weight: bold;  border-top: 1px solid Black; margin-top: 5px;">' . TEXT_INFORMATIONS . '</div><br />');
            $contents[] = array('text'  => '' . TEXT_DATE_ADDED . ' ' . xtc_date_short($pInfo->products_date_added) . '');
            ...
            $contents[] = array('text' => '<br />' . $price_string.  '<br />' . TEXT_PRODUCTS_DISCOUNT_ALLOWED_INFO . '&nbsp;' . $pInfo->products_discount_allowed . '<div style="padding-left: 30px;">' .  TEXT_PRODUCTS_QUANTITY_INFO . '&nbsp;' . (double)$pInfo->products_quantity . '');
            // END IN-SOLUTION

            //$contents[] = array('text' => '<br />' . TEXT_PRODUCTS_PRICE_INFO . ' ' . $currencies->format($pInfo->products_price) . '<br />' . TEXT_PRODUCTS_QUANTITY_INFO . ' ' . $pInfo->products_quantity);
            $contents[] = array('text' => '<br />' . TEXT_PRODUCTS_AVERAGE_RATING . ' ' . number_format((double)$pInfo->average_rating, 2) . ' %');
            $contents[] = array('text' => '' . TEXT_PRODUCT_LINKED_TO . '<br />' . xtc_output_generated_category_path($pInfo->products_id, 'product') . '');
            $contents[] = array('align' => 'center', 'text' => '' . xtc_product_thumb_image($pInfo->products_image, $pInfo->products_name)  . '<br />' . $pInfo->products_image.'');
          }          
        } else { 
