...
if ($_GET['store'] == 'true')
{
    ...
    $orderErg = mysql_fetch_assoc($orderQuery);
...
<td class="boxCenter" width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="pageHeading">mediafinanz - Forderung eintragen</td>
        ...
        <table border="0" cellspacing="0" cellpadding="2">
          <tr>
            ...
            if (!empty($order) && $_GET['store'] != 'true')
            {
                ...
                <form action="mediafinanz.php?action=process_claim&store=true&oID=<?php echo $ordersId; ?>" method="POST" name="newClaim">
                     ...
                     <table border="0" cellspacing="1" cellpadding="3" class="smalltext">
                      ...
                      <tr bgcolor="white">
                        <td><b>Rechngsnummer</b></td>
                        <td><b><?php echo $order['orderId']; ?></b></td>
                      </tr>
                      <tr bgcolor="white">
                        <td><b>Ursprungsbetrag</b></td>
                      ...
                      <tr bgcolor="white">
                        <td><b>Forderungstyp</b></td>
                        <td><b>
                            <select name="claimType">
                                <option value="1">Ware verkauft</option>
                                <option value="2">Ware per Vorkasse verkauft</option>
                                <option value="3">Dienstleistung erbracht </option>
                            </select>
                        </b></td>
                        </tr>
                      <tr bgcolor="white">
                        <td valign="top"><b>Begr&uuml;ndung<br/>(Wird so abgedruckt)</b></td>
                        <td>
                        <?php
                            $productString = '';
            ...
            elseif ($_GET['store'] == 'true')
            {
                if ($result['success'])
                {
                    echo 'Forderung &uuml;bergeben<br/ > das Aktenzeichen lautet: '.$result['fileNumber'].'<br/> <a href="mediafinanz.php?action=claims">zur&uuml;ck</a>';
                }
                else
                {
                    $path = parse_url($_SERVER['HTTP_REFERER']);
                    echo $result['error'].'<br/> <a href="mediafinanz.php?'.$path['query'].'">zur&uuml;ck</a>';
                }
            }
            else
            {
                echo 'Kein Datensatz mit dieser Rechnungs Id!';
            }
