 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: MagnaUpdater.php 1362 2011-11-16 16:19:58Z derpapst $
 *
...
class MagnaUpdater	{
	...
	public function checkMinimalFilePermissions($list = 'permissions.list') {
		...
		if (file_exists($this->paths['DIR_MAGNALISTER'].$list)) {	
			$fileList = $this->getFileList($this->paths['DIR_MAGNALISTER'].$list);
			if (is_array($fileList) && !empty($fileList)) {
				foreach ($fileList as $file => $md5hash) {
					if (!is_writable($this->paths['DIR_MAGNALISTER'].$file)) {
						$this->updaterAllErrors[] = array(
	...
	public function checkFilePermissions() {
		...
		if (file_exists($this->paths['DIR_MAGNALISTER'].'specialfiles.list')) {
			$fileList = $this->getFileList($this->paths['DIR_MAGNALISTER'].'specialfiles.list');
			if (is_array($fileList) && !empty($fileList)) {
	...
	private function cleanDir($filesToKeep = array(), $subdirPath = '') {
		...
		$saveFiles = array('.' => true, '..' => true, '.svn' => true);
		$chmodDirs = array();
