 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: DeletedView.php 1170 2011-07-30 15:12:50Z derpapst $
 *
...
class DeletedView {
	...
	public function __construct($settings = array()) {
		...
		$this->deletedLog = MagnaDB::gi()->fetchArray('
			SELECT csd.id, '.($artNr ? 'p.products_id' : 'csd.products_id').', csd.old_price,
			       csd.timestamp AS date, pd.products_name AS itemname 
			  FROM '.TABLE_MAGNA_CS_DELETEDLOG.' csd, '.($artNr ? TABLE_PRODUCTS.' p, ' : '').TABLE_PRODUCTS_DESCRIPTION.' pd
			 WHERE csd.mpID=\''.$this->magnasession['mpID'].'\' AND
			 		'.($artNr 
			 			? 'csd.products_model=p.products_model AND p.products_id=pd.products_id' 
			 			: 'csd.products_id=pd.products_id'
			 		).' AND
