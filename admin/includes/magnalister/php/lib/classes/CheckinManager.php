 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: CheckinManager.php 1474 2011-12-31 17:04:42Z derpapst $
 *
	...
	protected function renderTemplateSelector () {
		...
		$html = '
			<form id="templateSelection" name="templateSelection" method="POST" action="'.toURL($this->_url, array('mode'=>'checkin')).'">
				<label>'.ML_LABEL_USE_TEMPLATE.' <span id="template_info" class="gfxbutton info"></span></label>:
		...
		if (!empty($templates)) {
			...
			foreach ($templates as $template) {
				$html .= '<option value="'.$template['tID'].'"'.((isset($loadedTmpl['tID']) && ($loadedTmpl['tID'] == $template['tID'])) ? (' selected="selected"') : '').'>'.
					$template['title'].
				'</option>';
			}
		} else {
			$html .= '<option value="-1">'.ML_LABEL_NO_TEMPLATES_YET.'</option>';
		}
		$html .= '
				</select>
				<input type="submit" class="button" value="'.ML_BUTTON_LABEL_GO.'"/>
				<a id="editTemplates" href="'.toURL($this->_url, array('view'=>'administrate')).'" title="'.ML_BUTTON_LABEL_ADMINISTRATE_TEMPLATES.'">
					<img src="'.DIR_MAGNALISTER_IMAGES.'emblem-system.png" alt="'.ML_BUTTON_LABEL_ADMINISTRATE_TEMPLATES.'"/>
				</a>
						...
						<table class="right"><tbody>
							<tr>
								<td class="textleft">
									...
									'.(MAGNA_DEBUG ? '<input type="button" class="button smallmargin" style="margin-top: -21px; position: absolute; right: 30px;" '.
...
<?php
		...
		/* Category - Product - Overview View */
			...
			if (array_key_exists('selectTemplate', $_POST) && $_POST['selectTemplate'] > 0) {
			...
			if ($this->isAjax) {
				...
				return $this->renderTemplateSelector() . $aCV->printForm();
			}
		}
	}
}
