 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: Configurator.php 1532 2012-01-24 21:15:59Z MaW $
 *
				...
				$html .= '
					...
					<script type="text/javascript">/*<![CDATA[*/
						$(document).ready(function() {
							...
							var dateFormat'.$this->curInput['key'].' = $("#config_'.$idkey.'_visual").datepicker("option", "dateFormat");
							'.(!empty($default) ? '
	...
	public function renderConfigForm() {
		...
		foreach ($this->form as $section) {
			...
			foreach ($section['fields'] as $item) {
				...
				if (array_key_exists('expertsetting', $item) && ($item['expertsetting']) 
					...
					$html .= '
						<tr class="conf">
							'.(!empty($item['label'])
								? (
									'<th class="label">'.
									'<label for="config_'.$idkey.'"'.(in_array($item['key'], $this->missingConfigKeys) ? ' class="missing"' : '').'>'.
									$item['label'].
									'</label></th>' 
								)
								: ''
        ...
        if (    ('tinyMCE' == getDBConfigValue('general.editor',0,'tinyMCE'))
			...
			echo '
			<script type="text/javascript" src="includes/magnalister/js/tiny_mce/tiny_mce.js"></script>';
			ob_start();?>
	        <script type="text/javascript">/*<![CDATA[*/
