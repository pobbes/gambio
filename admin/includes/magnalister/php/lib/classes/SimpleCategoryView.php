 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: SimpleCategoryView.php 1367 2011-11-16 23:04:50Z derpapst $
 *
...
define('SIMPLE_CHECKBOXES', false);
...
//		$_MagnaSession[$_MagnaSession['currentPlatform']]['selection'][$this->settings['selectionName']] = array();
		...
		if (!SIMPLE_CHECKBOXES) {
			...
			$this->allowedProductIDs = $this->getChildProductsOfThisLevel($this->cPath);
			echo print_m($this->allowedProductIDs, '$this->allowedProductIDs');
		}
		// echo print_m($this->allowedProductIDs, '$this->allowedProductIDs');

		$this->selectProducts();
		if (SIMPLE_CHECKBOXES && !$this->isAjax) {
			echo print_m($_POST, '$_POST');
			echo print_m(MagnaDB::gi()->fetchArray('
					SELECT pID
					  FROM '.TABLE_MAGNA_SELECTION.'
					 WHERE mpID=\''.$this->_magnasession['mpID'].'\' AND
					       selectionname=\''.$this->settings['selectionName'].'\' AND
		...
		$queryStr = '
		   SELECT c.categories_id, cd.categories_name, c.categories_image, c.parent_id
  	        ...
  	        WHERE c.categories_id = cd.categories_id AND 
  	              ...
  	              '.(((SHOPSYSTEM != 'oscommerce') && $this->showOnlyActiveProducts) ? 'categories_status<>0 AND' : '').'
	...
	private function getCategoryCheckboxState($id) {
		if (!array_key_exists($id, $this->categoryCheckboxStateCache)) {
			$nProductsInCat = count($this->list['categories'][$id]['allproductsids']);
			$nProductsAddedFromCat = found_in_array($this->list['categories'][$id]['allproductsids'], $this->selection);
	
			if ($nProductsAddedFromCat == 0) {
				$this->categoryCheckboxStateCache[$id] = array(
					'state' => 'unchecked',
					'add' => true
				);
			} else if ($nProductsAddedFromCat < $nProductsInCat) {
				$this->categoryCheckboxStateCache[$id] = array(
					'state' => 'semichecked',
					'add' => true
				);
			} else if ($nProductsInCat == $nProductsAddedFromCat) {
				$this->categoryCheckboxStateCache[$id] = array(
					'state' => 'checked',
					'add' => false
				);
	...
	private function categorySelector($cID) {
		...
		//SIMPLE_CHECKBOXES
		return '<input type="'.((SIMPLE_CHECKBOXES) ? 'checkbox' : ($this->settings['ajaxSelector'] ? 'button' : 'submit')).'" '.
	...
	private function topSelectionButtons() {
		...
		return $addFields.'
			<input id="selectAll" type="'.((SIMPLE_CHECKBOXES) ? 'checkbox' : ($this->settings['ajaxSelector'] ? 'button' : 'submit')).'" '.
	...
	public function printForm() {
		...
		$html = '
		'.((SIMPLE_CHECKBOXES) 
			? '<form class="categoryView" id="actionForm" name="actionForm" action="'.toURL($this->url, $this->action).'" method="post">'
