 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: functionLib.php 1571 2012-02-24 16:10:22Z MaW $
 *
...
function _browser($a_browser = false, $a_version = false, $name = false, $d_name = false, $ret_ary = false) {
	...
	$platforms = array(
		'linux' => _browser_w('linux'),
		'mac' => array('macintosh', 'mac platform x', 'mac os x'),
		'windows' => _browser_w('windows win32')
	);
 
	$user_browser = strtolower(isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '');
	...
	foreach (_browser_w('user_browser a_browser a_version name d_name') as $row) {
		$vrow = $$row;
		if (is_string($vrow)) {
			$$row = strtolower($vrow);
		}
	...
	foreach (_browser_w($browser_list) as $row) {
	...
	$this_type = '';
	if (!empty($this_browser)) {
		foreach ($browser_type as $type => $browsers) {
			foreach (_browser_w($browsers) as $row) {
	...
	if ($name !== false && $ret_ary === false) {
		...
		foreach ($s_data as $row) {
			if (!empty($row)) $s_browser .= (($s_browser != '') ? ' ' : '') . $row;
		}
...
/*
 EOF XBD, Extended Browser Detection.
*/
...
function stripEvilBlockTags($str) {
	$str = str_replace("\r", "\n", str_replace("\r\n", "\n", $str));
