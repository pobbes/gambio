...
class ApplyCategoryView extends SimpleCategoryView {
			...
			if ($itemsApplied['itemsCount'] == 0) {
				...
				return '
					<td title="'.ML_AMAZON_LABEL_APPLY_NOT_PREPARED.'">'.
						html_image(DIR_MAGNALISTER_IMAGES . 'status/grey_dot.png', ML_AMAZON_LABEL_APPLY_NOT_PREPARED, 12, 12).
					'</td>';
			}
			if ($itemsApplied['incompleteCount'] == $totalItems) {
				...
				return '
					<td title="'.ML_AMAZON_LABEL_APPLY_PREPARE_INCOMPLETE.'">'.
						html_image(DIR_MAGNALISTER_IMAGES . 'status/red_dot.png', ML_AMAZON_LABEL_APPLY_PREPARE_INCOMPLETE, 12, 12).
					'</td>';
			}
			if (($itemsApplied['itemsCount'] == $totalItems) && ($itemsApplied['incompleteCount'] == 0)) {
				...
				return '
					<td title="'.ML_AMAZON_LABEL_APPLY_PREPARE_COMPLETE.'">'.
						html_image(DIR_MAGNALISTER_IMAGES . 'status/green_dot.png', ML_AMAZON_LABEL_APPLY_PREPARE_COMPLETE, 12, 12).
					'</td>';
			}
			if ($itemsApplied['itemsCount'] > 0) {
				...
				return '
					<td title="'.ML_AMAZON_LABEL_APPLY_PREPARE_INCOMPLETE.'">'.
						html_image(DIR_MAGNALISTER_IMAGES . 'status/yellow_dot.png', ML_AMAZON_LABEL_APPLY_PREPARE_INCOMPLETE, 12, 12).
					'</td>';
			}
		}
		return '
	...
	public function getLeftButtons() {
		return '<input type="submit" class="button" value="'.ML_AMAZON_BUTTON_APPLY_DELETE.'" id="removeapply" name="removeapply"/>';
	}
