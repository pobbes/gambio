 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: AmazonCategoryView.php 1123 2011-06-29 12:11:31Z MaW $
 *
		...
		if (($itemsFailed == 0) && ($itemsMatched == 0)) { /* Nichts gematched und auch kein matching probiert */
			return $html.'
				<td title="'.ML_AMAZON_CATEGORY_MATCHED_NONE.'">'.
					html_image(DIR_MAGNALISTER_IMAGES . 'status/grey_dot.png', ML_AMAZON_CATEGORY_MATCHED_NONE, 12, 12).
				'</td>';
		}
		if ($itemsFailed == $totalItems) { /* Keine gematched */
			return $html.'
				<td title="'.ML_AMAZON_CATEGORY_MATCHED_FALUTY.'">'.
					html_image(DIR_MAGNALISTER_IMAGES . 'status/red_dot.png', ML_AMAZON_CATEGORY_MATCHED_FALUTY, 12, 12).
				'</td>';
		}
		if ($itemsMatched == $totalItems) {  /* Alle Items in Category gematched */
			return $html.'
				<td title="'.ML_AMAZON_CATEGORY_MATCHED_ALL.'">'.
					html_image(DIR_MAGNALISTER_IMAGES . 'status/green_dot.png', ML_AMAZON_CATEGORY_MATCHED_ALL, 12, 12).
				'</td>';
		}
		if (($itemsFailed > 0) || ($itemsMatched > 0)) { /* Einige nicht erfolgreich gematched */
			return $html.'
				<td title="'.ML_AMAZON_CATEGORY_MATCHED_INCOMPLETE.'">'.
					html_image(DIR_MAGNALISTER_IMAGES . 'status/yellow_dot.png', ML_AMAZON_CATEGORY_MATCHED_INCOMPLETE, 12, 12).
				'</td>';
		}
