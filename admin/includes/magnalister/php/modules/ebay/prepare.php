...
function eBayGetSelection() {
	...
	if ('artNr' == getDBConfigValue('general.keytype', '0')) { 
		...
		.' ms.mpID mpID, Title, Subtitle, '
		.' Description, pd.products_description description, PictureURL, GalleryURL, ConditionID,  '
	...
	# Preis und ggf. VPE wird erst beim Uebermitteln eingesetzt.
		$substitution = array(
			'#TITLE#' => fixHTMLUTF8Entities($dbSelection[0]['Title']),
		...
		$dbSelection[0]['Description'] = substitutePictures(substituteTemplate($eBayTemplate, $substitution),$dbSelection[0]['products_id'],$imagePath);
	}
	return $dbSelection;
}
	...
	if ($kind == 'view') {
		...
		switch ($_POST['action']) {
			...
			case 'extern': {
				...
				$shipProc = new eBayShippingDetailsProcessor($args, '', $tmpURL);
