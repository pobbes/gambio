 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: DeletedView.php 897 2011-04-05 02:58:22Z derpapst $
 *
...
class DeletedView {
	...
	public function __construct($marketplace, $settings = array()) {
		...
		if (isset($_POST['action'])) {
			...
			switch ($action) {
				case 'delete': {
					...
					MagnaDB::gi()->query('
						DELETE FROM '.TABLE_MAGNA_EBAY_DELETEDLOG.'
		...
		$this->numberofitems = (int)MagnaDB::gi()->fetchOne(
			'SELECT DISTINCT count(id) FROM '.TABLE_MAGNA_EBAY_DELETEDLOG.' WHERE `mpID`=\''.$this->magnasession['mpID'].'\''
		);
		...
		$this->deletedLog = MagnaDB::gi()->fetchArray('
			SELECT ed.id, '.($artNr ? 'p.products_id' : 'ed.products_id').', ed.Price, ed.ItemID,
			       ed.timestamp AS date, pd.products_name AS itemname 
			  FROM '.TABLE_MAGNA_EBAY_DELETEDLOG.' ed, '.($artNr ? TABLE_PRODUCTS.' p, ' : '').TABLE_PRODUCTS_DESCRIPTION.' pd
			 WHERE ed.mpID=\''.$this->magnasession['mpID'].'\' AND
			 		'.($artNr 
			 			? 'ed.products_model=p.products_model AND p.products_id=pd.products_id' 
			 			: 'ed.products_id=pd.products_id'
			 		).' AND
