...
class eBaySummaryView extends SimpleSummaryView {
	...
	public function __construct($settings = array()) {
		...
		$this->chineseStockSync = getDBConfigValue($this->marketplace.'.chinese.quantity.type', $this->mpID);
	}
	...
	protected function extendProductAttributes($pID, &$data) {
		...
		$mpID = $this->_magnasession['mpID'];
		$listingproperties = MagnaDB::gi()->fetchRow('SELECT Price, ListingType, IF(\'Chinese\'=ListingType, \'chinese\',\'fixed\') as listingsupertype, ListingDuration as duration FROM '.TABLE_MAGNA_EBAY_PROPERTIES.' WHERE products_id='.$pID.' AND mpID='.$this->_magnasession['mpID'].' ORDER BY products_id DESC LIMIT 1');
		...
		if(0.0 != $listingproperties['Price']) $data['price'] = $listingproperties['Price'];
			...
			switch ($quantityType) {
				case 'stock': {
					...
					if (getDBConfigValue(array($mp.'.usevariations', 'val'), $mpID, true))
						$data['quantity'] = (int)getProductVariationsQuantity($pID);
				...
				case 'stocksub': {
					if (getDBConfigValue(array($mp.'.usevariations', 'val'), $mpID, true))
						$data['quantity'] = (int)getProductVariationsQuantity($pID, getDBConfigValue($mp.'.'.$data['listingsupertype'].'.quantity.value', $mpID, 0));
					else $data['quantity'] = 0;
					if (0 == $data['quantity'])
					$data['quantity'] = (int)MagnaDB::gi()->fetchOne('SELECT products_quantity FROM '.TABLE_PRODUCTS.' WHERE products_id=\''.$pID.'\'') - getDBConfigValue($mp.'.'.$data['listingsupertype'].'.quantity.value', $mpID, 0);
					if ($data['quantity'] < 0) {
						$data['quantity'] = 0;
					}
					break;
	...
	protected function getAdditionalItemCells($key, $dbRow) {
		...
		if ($type == 'chinese') {
			$type = $this->chineseStockSync;
		} else {
			$type = $this->fixedPriceStockSync;
		}
		$stock = (($type == 'stock') || ($type == 'stocksub'));
			...
			<td><table class="nostyle"><tbody>
					<tr><td>'.ML_LABEL_NEW.':&nbsp;</td><td>
						<input type="text" id="price_'.$dbRow['products_id'].'"
					           ...
					           value="'.$this->simplePrice->setPrice($this->selection[$dbRow['products_id']]['price'])->formatWOCurrency().'"/>
