...
?>
              <tr>
                <td colspan="5"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td align="right" class="dataTableContent" valign="top"><?php echo $countries_split->display_count($countries_query_numrows, '20', $_GET['page'], TEXT_DISPLAY_NUMBER_OF_COUNTRIES); ?> <?php echo $countries_split->display_links($countries_query_numrows, '20', MAX_DISPLAY_PAGE_LINKS, $_GET['page']); ?></td>
					</tr>
<?php
?>					<tr>
                    <td align="right">
						<?php echo '<a class="button float_right" onClick="this.blur();" href="' . xtc_href_link(FILENAME_COUNTRIES, 'page=' . $_GET['page'] . '&action=new') . '">' . BUTTON_NEW_COUNTRY	. '</a>'; ?>
						<?php echo '<a class="button float_right" href="' . xtc_href_link(FILENAME_COUNTRIES, 'action=setstatus&flag=1&cID=' . $countries['countries_id'] . '&page='.$_GET['page']) . '">' . GM_BUTTON_ACTIVE		. '</a>'; ?>
						<?php echo '<a class="button float_right" href="' . xtc_href_link(FILENAME_COUNTRIES, 'action=setstatus&flag=0&cID=' . $countries['countries_id'] . '&page='.$_GET['page']) . '">' . GM_BUTTON_INACTIVE		. '</a>'; ?>
						<?php echo '<a class="button float_right" href="' . xtc_href_link(FILENAME_COUNTRIES, 'action=gm_set_dach_active&cID=' . $countries['countries_id'] . '&page='.$_GET['page']) . '">' . GM_BUTTON_DACH		. '</a>'; ?>
					</td>
					</tr>
...
<?php
  ...
  switch ($_GET['action']) {
    case 'new':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br /><div align="center"><input type="submit" class="button" onClick="this.blur();" value="' . BUTTON_INSERT . '"/><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_COUNTRIES, 'page=' . $_GET['page']) . '">' . BUTTON_CANCEL . '</a></div>');
      break;
    case 'edit':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br /><div align="center"><input type="submit" class="button" onClick="this.blur();" value="' . BUTTON_UPDATE . '"/><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_COUNTRIES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->countries_id) . '">' . BUTTON_CANCEL . '</a></div>');
      break;
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br /><div align="center"><input type="submit" class="button" onClick="this.blur();" value="' . BUTTON_DELETE . '"/><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_COUNTRIES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->countries_id) . '">' . BUTTON_CANCEL . '</a></div>');
      break;
    default:
      if (is_object($cInfo)) {
        $heading[] = array('text' => '<b>' . $cInfo->countries_name . '</b>');

        $contents[] = array('align' => 'center', 'text' => '<div align="center"><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_COUNTRIES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->countries_id . '&action=edit') . '">' . BUTTON_EDIT . '</a> <a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_COUNTRIES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->countries_id . '&action=delete') . '">' . BUTTON_DELETE . '</a></div>');
