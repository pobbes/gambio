...
defined('_VALID_XTC') or die('Direct Access to this location is not allowed.');
    ...
    $formActive   = getValue('YOOCHOOSE_ACTIVE', false);
    $formId       = getValue('YOOCHOOSE_ID', '');
    $formSecret   = getValue('YOOCHOOSE_SECRET', false);
	...
	if (isset($_POST['YOOCHOOSE_ID']) || isset($_POST['YOOCHOOSE_ACTIVE']) ) {
    	
    	$formActive = isset($_POST['YOOCHOOSE_ACTIVE']); // if not in POST -> unchecked.
...
?>
    ...
    <table class="dataTable" border="0" cellspacing="0" cellpadding="4" >
        <tr valign="top" bgcolor="<?php printRowColor() ?>">
            ...
            <td><input name='YOOCHOOSE_ACTIVE' type="checkbox" value='true' <? echo $formActive?'checked="checked"':''?>">
            <div class="desc"><?php echo sprintf(YOOCHOOSE_ACTIVE_DESC)?></div></td>
        </tr>
        <?php if ($formActive) { ?>
	        <tr valign="top" bgcolor="<?php printRowColor() ?>">            
	            <th><?php echo sprintf(YOOCHOOSE_ID_TITLE)?></th>
	            <td><input name='YOOCHOOSE_ID' type="text" value="<? echo htmlentities($formId)?>">
	            <div class="desc"><?php echo sprintf(YOOCHOOSE_ID_DESC, $regpage)?></div></td>           
	        </tr>
	        <tr valign="top" bgcolor="<?php printRowColor() ?>">
	            <th><?php echo sprintf(YOOCHOOSE_SECRET_TITLE)?></th>
	            <td><input size="40" name='YOOCHOOSE_SECRET' type="text" value="<? echo htmlentities($formSecret)?>">
            ...
            <?php if (! (isset($_POST['YOOCHOOSE_ACTIVE']) && ! isset($_POST['YOOCHOOSE_ID']))) { ?>
		        <tr valign="top" bgcolor="<?php printRowColor() ?>">            
		            ...
		            <td><input size="40" name='NOT_USED' type="text" value="<? echo htmlentities($formLicense)?>" readonly="readonly">
...
<?php 
    ...
    function loadLicense($confServer, $userId, $password) {
        ...
        $license_json = load_json_url_ex(
            $confServer."/rest/".$userId."/license.json", 
