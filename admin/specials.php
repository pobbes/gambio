<?php
/* --------------------------------------------------------------
   specials.php 2011-07-12 gambio
   Gambio GmbH
   http://www.gambio.de
...
?>
      <tr ><form name="new_special" <?php echo 'action="' . xtc_href_link(FILENAME_SPECIALS, xtc_get_all_get_params(array('action', 'info', 'sID')) . 'action=' . $form_action, 'NONSSL') . '"'; ?> method="post"><?php if ($form_action == 'update') echo xtc_draw_hidden_field('specials_id', $_GET['sID']); ?>
        <td class="gm_border dataTableRow"><table border="0" cellspacing="0" cellpadding="2">
	   ...
	   ?>
          ...
          <tr>
            <td class="main" align="right" valign="top">
					<?php echo (($form_action == 'insert') ? '<input style="float:left" type="submit" class="button" onClick="this.blur();" value="' . BUTTON_INSERT . '"/>' : '<input style="float:left" type="submit" class="button" onClick="this.blur();" value="' . BUTTON_UPDATE . '"/>'). '<a style="float:left" class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_SPECIALS, 'page=' . $_GET['page'] . '&sID=' . $_GET['sID']) . '">' . BUTTON_CANCEL . '</a>'; ?></td>
          </tr>
        </table>
...
?>
                ...
                <td  class="dataTableContent"><span class="oldPrice">
...
<?php
      if ($specials['status'] == '1') {
        echo xtc_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . xtc_href_link(FILENAME_SPECIALS, 'action=setflag&flag=0&id=' . $specials['specials_id'] . "&page=" . $_GET['page'], 'NONSSL') . '">' . xtc_image(DIR_WS_IMAGES . 'icon_status_red_light.gif', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
      } else {
        echo '<a href="' . xtc_href_link(FILENAME_SPECIALS, 'action=setflag&flag=1&id=' . $specials['specials_id']. "&page=" . $_GET['page'], 'NONSSL') . '">' . xtc_image(DIR_WS_IMAGES . 'icon_status_green_light.gif', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . xtc_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_ICON_STATUS_RED, 10, 10);
      }
?></td>
                <td class="dataTableContent"><?php if ( (is_object($sInfo)) && ($specials['specials_id'] == $sInfo->specials_id) ) { echo xtc_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . xtc_href_link(FILENAME_SPECIALS, 'page=' . $_GET['page'] . '&sID=' . $specials['specials_id']) . '">' . xtc_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
      </tr>
<?php
    }
?>
    ...
    default:
      if (is_object($sInfo)) {
        ...
        $contents[] = array('text' => '' . TEXT_INFO_NEW_PRICE . ' ' . $xtPrice->xtcFormat($sInfo->specials_new_products_price,true));
				// BOF GM_MOD:
        if($sInfo->products_price > 0) $contents[] = array('text' => '' . TEXT_INFO_PERCENTAGE . ' ' . number_format((double)100 - (($sInfo->specials_new_products_price / $sInfo->products_price) * 100)) . '%');
