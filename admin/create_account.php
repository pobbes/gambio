<?php
/* --------------------------------------------------------------
   create_account.php 2012-04-03 gm
   Gambio GmbH
   http://www.gambio.de
...
?>
	...
	if ($error == true) {
		if ($entry_company_error == true) {
			// BOF GM_MOD:
			echo xtc_draw_input_field('entry_company', $entry_company, 'maxlength="255"').'&nbsp;'.ENTRY_COMPANY_ERROR;
		} else {
			// BOF GM_MOD:
			echo xtc_draw_input_field('entry_company', $entry_company, 'maxlength="255"');
		}
	} else {
		// BOF GM_MOD:
		echo xtc_draw_input_field('entry_company', $entry_company, 'maxlength="255"');
...
?>
		...
		if ($error == true) {
			if ($entry_vat_error == true) {
				echo xtc_draw_input_field('customers_vat_id', $customers_vat_id, 'maxlength="32"').'&nbsp;'.ENTRY_VAT_ID_ERROR;
			} else {
...
?></td>
      ...
      <tr>
        <td align="right" class="main">
			<?php 
			echo '<input style="float:right"  type="submit" class="button" onClick="this.blur();" value="' . BUTTON_INSERT . '" />
			<a style="float:right" class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_CUSTOMERS, xtc_get_all_get_params(array('action'))) .'">' . BUTTON_CANCEL . '</a>'; ?></td>
      </tr></form>
      </table></td>
      </tr>
    </table></td>
