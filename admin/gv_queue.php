...
/* --------------------------------------------------------------
  ...
  if ($_GET['action']=='confirmrelease' && isset($_GET['gid'])) {
	...
	elseif($_GET['action']=='delete' && isset($_GET['gid'])){
	...
	switch ($_GET['action']) {
		...
		default:
			...
			if(!empty($gv_query_numrows)){
				$heading[] = array('text' => '[' . $gInfo->unique_id . '] ' . xtc_datetime_short($gInfo->date_created) . ' ' . $currencies->format($gInfo->amount));
				$contents[] = array ('align' => 'center', 'text' => '<div style="padding-top: 5px; font-weight: bold; ">' . TEXT_MARKED_ELEMENTS . '</div><br />');           
				$contents[] = array('align' => 'center','text' => '<div align="center"><a class="button" onClick="this.blur();" href="' . xtc_href_link('gv_queue.php','action=release&gid=' . $gInfo->unique_id,'NONSSL'). '">' . BUTTON_RELEASE . '</a><a class="button" onclick="return confirm(\'' . GM_GV_DELETE . '\')" href="' . xtc_href_link('gv_queue.php','action=delete&gid=' . $gInfo->unique_id,'NONSSL'). '">' . BUTTON_DELETE . '</a></div>');
			}
			// EOF GM_MOD
		break;
	}
