...
if ((($_GET['action'] == 'edit') || ($_GET['action'] == 'update_order')) && ($_GET['oID'])) {
	...
	if (!xtc_db_num_rows($orders_query)) {
		$orders_query = xtc_db_query("select orders_id from ".TABLE_ORDERS." where gm_orders_id = '".xtc_db_input($oID)."' LIMIT 1");
		if (!xtc_db_num_rows($orders_query)) {
		$order_exists = false;
		...
		default:
			if (is_object($oInfo)) {
				...
				if ($oInfo->payment_method == 'billsafe_2') { 
					$contents[] = array ('align' => 'center', 'text' => '<div align="center"><a class="button" href="billsafe_orders_2.php?oID='.$oInfo->orders_id.'">BillSAFE Details</a></div>'); 
				}
				// bof
