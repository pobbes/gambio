<?php
/* --------------------------------------------------------------
   quantity_units.php 2011-02-21 ih
   Gambio GmbH
   http://www.gambio.de
...
function generateNewQuantityUnit()
{
  ...
  foreach($languages_installed as $l_key => $lang_data) {
    ...
    if ($lang_all || (!$lang_all && ($t_lang_id == $lang_shop))) {
      ...
      $html .= $icon.'&nbsp;'.'<input type="text" style="width:160px;" name="unitNew['.$t_lang_id.']">'."<br>\n";
    }
  }
  return $html;
}
  ...
  foreach ($quantity_unit_array as $f_key => $coo_quantity_unit) {
    ...
    foreach($languages_installed as $l_key => $lang_data) {
      ...
      if (($lang_all || (!$lang_all && ($t_lang_id == $lang_shop))) && !empty($t_quantity_unit_name_array[$t_lang_id])) {
        ...
        $html .= '<a href="'.xtc_href_link(FILENAME_QUANTITYUNITS, '', 'NONSSL').'?quantity_unit_id='.$t_quantity_unit_id.'&lang_all='.(int)$lang_all.'">('.$t_lang_code.') '.$t_mark.$t_quantity_unit_name_array[$t_lang_id].'</span></a>'."<br>\n";
      }
    }
  }
  return $html;
...
function generateQuantityUnitName()
{
  ...
  foreach ($quantity_unit_array as $f_key => $coo_quantity_unit) {
    ...
    foreach($languages_installed as $l_key => $lang_data) {
      ...
      if ($lang_all || (!$lang_all && ($t_lang_id == $lang_shop))) {
        $icon  = xtc_image(DIR_WS_LANGUAGES.$t_lang_dir.'/admin/images/'.$t_lang_img);
        $html .= $icon.'&nbsp;'.'<input type="text" style="width:160px;" name="unitName['.$quantity_unit_id.']['.$t_lang_id.']" value="'.$t_quantity_unit_name_array[$t_lang_id].'">'."&nbsp;";
        ...
        <tr>
          <td valign="top" class="main">
            <div id="gm_box_content">
              <?php echo TEXT_NEW_QUANTITYUNIT; ?>:<br>
