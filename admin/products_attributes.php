<?php
/* --------------------------------------------------------------
   products_attributes.php 2012-03-19 gm
   Gambio GmbH
   http://www.gambio.de
   ...
   --------------------------------------------------------------*/
	...
	if ($_GET['action'])
	{
		...
		{
			$gm_searchoption = '&searchoption=' . $_GET['searchoption'];
		}

		if(!empty($_GET['search_optionsname']))
		{
			$gm_search_optionsname = '&search_optionsname=' . $_GET['search_optionsname'];
		}
		...
		switch($_GET['action']) 
		{
										...
										TABLE_PRODUCTS_ATTRIBUTES_DOWNLOAD . " 
											VALUES  
												(
													...
													'" . $_POST['products_attributes_filename'] . "', 
													'" . $_POST['products_attributes_maxdays'] . "', 
													'" . $_POST['products_attributes_maxcount'] . "'
												)
								");
				}
				...
				for ($i = 0, $n = sizeof($languages); $i < $n; $i ++) 
				{
					...
					xtc_db_query("
									UPDATE " . 
										TABLE_PRODUCTS_OPTIONS . " 
									SET 
										products_options_name	= '" . $option_name[$languages[$i]['id']] . "' 
									WHERE
										products_options_id		= '" . $_POST['option_id'] . "' 
									AND 
					...
					if(!empty($gm_filename))
					{
						...
						xtc_db_query("
										UPDATE " . 
											TABLE_PRODUCTS_OPTIONS_VALUES . " 
										SET 
											gm_filename = 'ATTRIBUTE_" . $_POST['value_id'] . strrchr($gm_filename, '.') . "' 
										WHERE 
											products_options_values_id = '" . $_POST['value_id'] . "' 
										AND
											language_id = '" . $languages[$i]['id'] . "'
			...
			case 'update_product_attribute':
				...
				xtc_db_query("
								...
								WHERE
									products_attributes_id = '" . $_POST['attribute_id'] . "'
							");
			...
			case 'delete_option':
				...
				$del_options = xtc_db_query(
											...
											FROM " . 
												TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS_OPTIONS . "
											WHERE
												products_options_id = '" . $_GET['option_id'] . "'
											");
				...
				{  
					xtc_db_query("
								...
								FROM " . 
									TABLE_PRODUCTS_OPTIONS_VALUES . " 
								WHERE 
									products_options_values_id = '" . $del_options_values['products_options_values_id'] . "'
							");
				}
			...
			case 'delete_value':
				...
				$gm_get_filename = xtc_db_query("
												...
												FROM " . 
													TABLE_PRODUCTS_OPTIONS_VALUES . " 
												WHERE
													products_options_values_id = '" . $_GET['value_id'] . "' 
												LIMIT 1
											");
			...
			case 'delete_attribute':
				...
				xtc_db_query("
								...
								FROM " . 							
									TABLE_PRODUCTS_ATTRIBUTES . " 
								WHERE 
									products_attributes_id = '" . $_GET['attribute_id'] . "'
							");
...
<body marginwidth="0" marginheight="0" topmargin="0" bottommargin="0" leftmargin="0" rightmargin="0" bgcolor="#FFFFFF">
	...
	<tr>
		...
		<div class="pageHeading" style="float:left; background-image:url(images/gm_icons/artkatalog.png)">
...
<!-- OPTIONS BOF //-->
							...
							<!-- 							
								<tr>
									...
									<td>
										&nbsp;
										<?php 
											echo xtc_image(DIR_WS_IMAGES . 'pixel_trans.gif', '', '1', '53'); ?>
										&nbsp;
									</td>
								</tr>
							-->
										...
										<table border="0" width="100%" cellspacing="0" cellpadding="2">
											...
											<tr>
												<td	align="left" colspan="3" class="main">
													<br />
													<?php 
														echo xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_PRODUCTS_ATTRIBUTES, '&value_page=' . $_GET['value_page']	. '&attribute_page=' . $attribute_page,	'NONSSL'));
													?>
													&nbsp;
												</td>
											</tr>
											...
											<tr>
												<td	class="main" align="left" colspan="3">
													<br />
													<?php 
														echo xtc_button_link(BUTTON_DELETE, xtc_href_link(FILENAME_PRODUCTS_ATTRIBUTES, 'action=delete_option&option_id=' . $_GET['option_id'], 'NONSSL'), 'style="float:left"');
													?>
													<?php 
														echo xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_PRODUCTS_ATTRIBUTES,	'&order_by=' . $order_by . '&page=' . $page, 'NONSSL'), 'style="float:left"');
													?>
												</td>
											</tr>
										<?php
									...
									<?php
										...
										if (isset ($_GET['searchoption'])) 
										{
											$options = "SELECT
														...
														AND
															(
																products_options_name LIKE '%" . $_GET['searchoption'] . "%'
																OR 
																products_options_id = '" . trim($_GET['searchoption']) . "'
															)
														ORDER BY " . 
															$option_order_by;
										} 
								...
								?>
									...
									<td align="center" class="smallText">
										&nbsp;
										<?php 
											echo xtc_button(BUTTON_UPDATE, 'submit', 'style="float:right"');									
										?>
										<?php 
											echo xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_PRODUCTS_ATTRIBUTES, 'option_page=' . $_GET['option_page'], 'NONSSL'), 'style="float:right"');
										?>
									</td>
								<?php
										echo '</form>' . "\n";
								...
								?>
									...
									<td align="center" class="smallText">
										...
										<?php
											...
											echo xtc_button_link(BUTTON_EDIT, xtc_href_link(FILENAME_PRODUCTS_ATTRIBUTES, 'action=update_option&option_id=' . $options_values['products_options_id'] . '&option_order_by='	. $option_order_by . '&option_page=' . $option_page . $gm_searchoption, 'NONSSL'),	'style="float:right"');
										?>
										<?php 
											echo xtc_button_link(BUTTON_DELETE, xtc_href_link(FILENAME_PRODUCTS_ATTRIBUTES, 'action=delete_product_option&option_id=' . $options_values['products_options_id'], 'NONSSL'), 'style="float:right"');
										?>
									</td>								
								<?php
									}
							...
							?>
									...
									<td align="center" class="smallText">
										&nbsp;
										<?php 
											echo xtc_button(BUTTON_INSERT, 'submit', 'style="float:right"'); 
										?>
										&nbsp;
									</td>
									<?php
						...
						?>
									...
									<table border="0" width="100%" cellspacing="0" cellpadding="2">
										...
										<tr>
											<td class="main" align="left" colspan="3">
												<br />
												<?php 
													echo xtc_button_link(BUTTON_DELETE, xtc_href_link(FILENAME_PRODUCTS_ATTRIBUTES, 'action=delete_value&value_id=' . $_GET['value_id'], 'NONSSL'), 'style="float:left"');
												?>
												<?php 
													echo xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_PRODUCTS_ATTRIBUTES, '&option_page=' . $option_page . '&value_page=' . $_GET['value_page']	. '&attribute_page=' . $attribute_page,	'NONSSL'), 'style="float:left"');
												?>
											</td>
										</tr>
										<?php
								...
								<?php
									...
									if (!empty ($_GET['search_optionsname'])) 
									{
										$values = "
													...
													AND 
														(
															po.products_options_name LIKE '%" . $_GET['search_optionsname'] . "%' 
														OR 
															pov.products_options_values_name LIKE '%" . $_GET['search_optionsname'] . "%'
														OR
															pov.products_options_values_id = '" . trim($_GET['search_optionsname']) . "'
														)
													ORDER BY
														pov.products_options_values_id";
									} 
									...
									{										
										$values = "
												...
												WHERE
													pov.language_id = '" . $_SESSION['languages_id'] . "'	
												AND
													pov.products_options_values_id = '" . trim($_GET['value_id']) . "'
												ORDER BY
													pov.products_options_values_id";
									}
									else 
							...
							?>
								...
								<?php
									// BOF GM_MOD
									$gm_filename = array();
									$gm_get_filename = xtc_db_query("SELECT gm_filename	FROM " . TABLE_PRODUCTS_OPTIONS_VALUES . " WHERE products_options_values_id = '" . $_GET['value_id'] . "' LIMIT	1");
								...
								<td	class="smallText">
									...
									&nbsp;
									<?php echo xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_PRODUCTS_ATTRIBUTES, 'value_page='.$_GET['value_page'], 'NONSSL')); 
									?>
									&nbsp;
								</td>
								<?php
								...
								<?php
									// BOF GM_MOD
									$gm_filename = array();
									$gm_get_filename = xtc_db_query("SELECT gm_filename	FROM " . TABLE_PRODUCTS_OPTIONS_VALUES . " WHERE products_options_values_id = '" . $values_values["products_options_values_id"]	. "' LIMIT 1");
									if(xtc_db_num_rows($gm_get_filename) == 1) $gm_filename = xtc_db_fetch_array($gm_get_filename);
								?>
								<td	class="smallText">
									&nbsp;
								...
								<td	align="center" class="smallText">
									...
									<?php
										...
										echo xtc_button_link(BUTTON_EDIT, xtc_href_link(FILENAME_PRODUCTS_ATTRIBUTES, 'action=update_option_value&value_id=' . $values_values['products_options_values_id'] . '&value_page=' . $_GET['value_page'] . $gm_search_optionsname, 'NONSSL')); 
									?>
									&nbsp;
									&nbsp;
									<?php 
										echo xtc_button_link(BUTTON_DELETE, xtc_href_link(FILENAME_PRODUCTS_ATTRIBUTES, 'action=delete_option_value&value_id='	. $values_values['products_options_values_id'], 'NONSSL')); 
									?>
									&nbsp;
								</td>
								<?php
...
<?php require(DIR_WS_INCLUDES .	'application_bottom.php'); ?>de