...
$gmStatusbar = MainFactory::create_object('GMStatusbar');
	...
	switch ($_GET['function']) {
		case 'delete' :
			...
			xtc_db_query("DELETE FROM personal_offers_by_customers_status_".(int) $_GET['statusID']."
						                     ...
						                     AND quantity    = '".(double) $_GET['quantity']."'");
			break;
	}
			...
			if (isset($_POST['multi_xtb'])) {
				...
				if (is_array($_POST['multi_products'])) {
					...
					foreach($x as $products_id) {
						$q = xtc_db_query("select products_quantity from ".TABLE_PRODUCTS." where products_id = '".xtc_db_input($products_id)."'");
						$p = xtc_db_fetch_array($q);
						if($MULTI_ONLYONSTOCK=='true'&&$p['products_quantity']<1) continue;
						$_SESSION['xtb1']['multi_xtb'][]=xtc_db_input($products_id);
					}
				}
				...
				if (is_array($_POST['multi_categories'])) {
					...
					function _xtb_reverse($category_id=0) {
						...
						$cp = xtc_db_query("select * from ".TABLE_CATEGORIES." where parent_id = '".xtc_db_input($category_id)."'");
						while($c=xtc_db_fetch_array($cp)) {
							$q = xtc_db_query("select p.products_id, p.products_quantity from ".TABLE_PRODUCTS." p, ".TABLE_PRODUCTS_TO_CATEGORIES." p2c where p.products_id = p2c.products_id and p.products_status = '1' and p2c.categories_id = '".xtc_db_input($c['categories_id'])."'");
					...
					foreach ($_POST['multi_categories'] AS $i=>$category_id) {
						$q = xtc_db_query("select p.products_id, p.products_quantity from ".TABLE_PRODUCTS." p, ".TABLE_PRODUCTS_TO_CATEGORIES." p2c where p.products_id = p2c.products_id and p.products_status = '1' and p2c.categories_id = '".xtc_db_input($category_id)."'");
						while($p = xtc_db_fetch_array($q)) {
							if($MULTI_ONLYONSTOCK=='true'&&$p['products_quantity']<1) continue;
							$_SESSION['xtb1']['multi_xtb'][$p['products_id']] = $p['products_id'];
						}
...
// delete 'feature setup' for product_id
if (isset($_POST['deleteSetup'])) {
  ...
  $coo_control->build_categories_index($prod_id);
  $coo_control = NULL;
}
...
if($_GET['action'] == 'insert_category' || $_GET['action'] == 'insert_product')
{
...
<html <?php echo HTML_PARAMS; ?>>
	<head>
        ...
        <style type="text/css">
            ...
            .product_tab_box input,
            .product_tab_box textarea{
                float: left;
                display: none;
            }
