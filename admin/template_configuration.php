<?php
/* --------------------------------------------------------------
   template_configuration.php 2012-04-18 gm
   Gambio GmbH
   http://www.gambio.de
...
class TemplateConfiguration
{
	...
	{
		$c_attr = mysql_real_escape_string($p_attr);
		$c_value = mysql_real_escape_string($p_value);
		...
		$t_sql = "SELECT
					...
					WHERE
						s.template_name = '" . mysql_real_escape_string(CURRENT_TEMPLATE) . "' AND
	...
	{
		$c_attr = mysql_real_escape_string(trim($p_attr));
		$c_old_value = mysql_real_escape_string(trim($p_old_value));
		$c_new_value = mysql_real_escape_string(trim($p_new_value));
	...
	if(isset($_POST['shop_align']))
	{
		...
		for($i = 0; $i < count($t_css_selectors_array); $i++)
		{
			$t_sql = "SELECT
						...
						WHERE
							s.template_name = '" . mysql_real_escape_string(CURRENT_TEMPLATE) . "' AND
							s.style_name = '" . mysql_real_escape_string($t_css_selectors_array[$i]) . "' AND
							s.gm_css_style_id = sc.gm_css_style_id AND
			...
			if(xtc_db_num_rows($t_result) == 0)
			{
				...
				if(xtc_db_num_rows($t_result) == 1)
				{
					...
					if(xtc_db_num_rows($t_result) == 1)
					{
						...
						$t_sql = "UPDATE gm_css_style_content
									SET style_value = '" . mysql_real_escape_string($t_margin_left) . "'
									WHERE gm_css_style_content_id = '" . (int)$t_margin_left_array['gm_css_style_content_id'] . "'";
						$t_result = xtc_db_query($t_sql);
						
						$t_sql = "UPDATE gm_css_style_content
									SET style_value = '" . mysql_real_escape_string($t_margin_right) . "'
			...
			elseif(xtc_db_num_rows($t_result) == 1)
			{
				...
				$t_sql = "UPDATE gm_css_style_content
									SET style_value = '0 " . mysql_real_escape_string($t_margin_right) . " 0 " .  mysql_real_escape_string($t_margin_left) . " 0'
...
<html>
	<head>
		...
		<script type="text/javascript"><!--
			...
			{
				$('input[name="cat_menu_left"]').click(function()
				{
					if($(this).attr('checked'))
					{
						$('input[name="cat_menu_classic"]').attr('checked', false);
					}
				});

				$('input[name="cat_menu_classic"]').click(function()
				{
					if($(this).attr('checked'))
					{
						$('input[name="cat_menu_left"]').attr('checked', false);
					}
				});
			});
