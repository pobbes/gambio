    ...
    switch ($_GET['action']) {
      ...
      case 'deleteconfirm':
        ...
        if ( (function_exists('imagecreate')) && ($banner_extension) ) {
          if (is_file(DIR_FS_CATALOG . 'cache/banner_infobox-' . $banners_id . '-' . FileLog::get_secure_token() .  '.' . $banner_extension)) {
            if (is_writeable(DIR_FS_CATALOG . 'cache/banner_infobox-' . $banners_id . '-' . FileLog::get_secure_token() .  '.' . $banner_extension)) {
              unlink(DIR_FS_CATALOG . 'cache/banner_infobox-' . $banners_id . '-' . FileLog::get_secure_token() .  '.' . $banner_extension);
            }
          }

          if (is_file(DIR_FS_CATALOG . 'cache/banner_yearly-' . $banners_id . '-' . FileLog::get_secure_token() .  '.' . $banner_extension)) {
            if (is_writeable(DIR_FS_CATALOG . 'cache/banner_yearly-' . $banners_id . '-' . FileLog::get_secure_token() .  '.' . $banner_extension)) {
              unlink(DIR_FS_CATALOG . 'cache/banner_yearly-' . $banners_id . '-' . FileLog::get_secure_token() .  '.' . $banner_extension);
            }
          }

          if (is_file(DIR_FS_CATALOG . 'cache/banner_monthly-' . $banners_id . '-' . FileLog::get_secure_token() .  '.' . $banner_extension)) {
            if (is_writeable(DIR_FS_CATALOG . 'cache/banner_monthly-' . $banners_id . '-' . FileLog::get_secure_token() .  '.' . $banner_extension)) {
              unlink(DIR_FS_CATALOG . 'cache/banner_monthly-' . $banners_id . '-' . FileLog::get_secure_token() .  '.' . $banner_extension);
            }
          }

          if (is_file(DIR_FS_CATALOG . 'cache/banner_daily-' . $banners_id . '-' . FileLog::get_secure_token() .  '.' . $banner_extension)) {
            if (is_writeable(DIR_FS_CATALOG . 'cache/banner_daily-' . $banners_id . '-' . FileLog::get_secure_token() .  '.' . $banner_extension)) {
              unlink(DIR_FS_CATALOG . 'cache/banner_daily-' . $banners_id . '-' . FileLog::get_secure_token() .  '.' . $banner_extension);
            }
          }
        }
      ...
      <tr><?php echo xtc_draw_form('new_banner', FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&action=' . $form_action, 'post', 'enctype="multipart/form-data"'); if ($form_action == 'update') echo xtc_draw_hidden_field('banners_id', $bID); ?>
        <td><table border="0" cellspacing="0" cellpadding="2" class="gm_border dataTableRow">
          ...
          <tr>
			...
			<?php echo (($form_action == 'insert') ? '<input style="float:right" type="submit" class="button" onClick="this.blur();" value="' . BUTTON_INSERT . '"/>' : '<input style="float:right" type="submit" class="button" onClick="this.blur();" value="' . BUTTON_UPDATE . '"/>'). '<a style="float:right" class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $_GET['bID']) . ' style="float:right">' . BUTTON_CANCEL . '</a>'; ?>
			</td>
          </tr>
        </table></td>
      </tr>
...
<?php
  ...
  switch ($_GET['action']) {
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br /><div align="center"><input type="submit" class="button" onClick="this.blur();" value="' . BUTTON_DELETE . '"/><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $_GET['bID']) . '">' . BUTTON_CANCEL . '</a></div>');
      break;
    default:
      if (is_object($bInfo)) {
        $heading[] = array('text' => '<b>' . $bInfo->banners_title . '</b>');
		$contents[] = array ('align' => 'center', 'text' => '<div style="padding-top: 5px; font-weight: bold; ">' . TEXT_MARKED_ELEMENTS . '</div><br />');           
        $contents[] = array('align' => 'center', 'text' => '<div align="center"><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $bInfo->banners_id . '&action=new') . '">' . BUTTON_EDIT . '</a><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $bInfo->banners_id . '&action=delete') . '">' . BUTTON_DELETE . '</a></div>');
        ...
        if ( (function_exists('imagecreate')) && ($banner_extension) ) {
          ...
          $contents[] = array('align' => 'center', 'text' => '<br />' . xtc_image(DIR_WS_CATALOG . 'cache/banner_infobox-' . $banner_id . '-' . FileLog::get_secure_token() . '.' . $banner_extension));
        } else {
