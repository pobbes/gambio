...
if ($_GET['action'] == "shipping_edit") {
	...
	if(file_exists(DIR_FS_LANGUAGES.$order->info['language'].'/modules/shipping/'.$module))
	{
		require (DIR_FS_LANGUAGES.$order->info['language'].'/modules/shipping/'.$module);
		if(defined('MODULE_SHIPPING_'.strtoupper($_POST['shipping']).'_TEXT_TITLE'))	
		{	
			...
			// BOF GM_MOD:
			$sql_data_array = array ('orders_id' => xtc_db_prepare_input($_POST['oID']), 'title' => xtc_db_prepare_input($shipping_text), 'text' => xtc_db_prepare_input($text), 'value' => xtc_db_prepare_input($_POST['value']), 'class' => 'ot_shipping', 'sort_order' => MODULE_ORDER_TOTAL_SHIPPING_SORT_ORDER);
...
if ($_GET['action'] == "ot_edit") {
	// BOF GM_MOD:
	$check_total_query = xtc_db_query("select orders_total_id from ".TABLE_ORDERS_TOTAL." where orders_id = '".$_POST['oID']."' and orders_total_id = '" . $_POST['otID'] . "' and class = '".$_POST['class']."'");
...
if ($_GET['action'] == "lang_edit") {
	...
	while ($order_total = xtc_db_fetch_array($order_total_query)) {
		if(file_exists(DIR_FS_LANGUAGES.$lang['directory'].'/modules/order_total/'.$order_total['class'].'.php'))
		{
...
if ($_GET['action'] == "product_delete") {
	...
	xtc_db_query("DELETE FROM orders_products_quantity_units WHERE orders_products_id = '" . xtc_db_input($_POST['opID']) . "'");
...
if ($_GET['action'] == "save_order") {
	// recreate order
	$coo_recreate_order = MainFactory::create_object('RecreateOrder', array($_POST['oID']));
    ...
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="0">
      <tr>
        <td width="100%" colspan="2">
			<div class="pageHeading" style="background-image:url(images/gm_icons/kunden.png)"><?php echo TABLE_HEADING; ?>
		</td>
      </tr>
	  <tr>
		  <td>
...
switch ($_GET['action']) {
	...
	default :
		if (is_object($order)) {
			...
			$contents[] = array ('align' => 'center', 'text' => '<br />'.TEXT_EDIT_ADDRESS.'<br /><div align="center"><a class="button" onClick="this.blur();" href="'.xtc_href_link(FILENAME_ORDERS_EDIT, 'edit_action=address&oID='.$_GET['oID']).'">'.BUTTON_EDIT.'</a></div><br /><br />');
			$contents[] = array ('align' => 'center', 'text' => '<br />'.TEXT_EDIT_PRODUCTS.'<br /><div align="center"><a class="button" onClick="this.blur();" href="'.xtc_href_link(FILENAME_ORDERS_EDIT, 'edit_action=products&oID='.$_GET['oID']).'">'.BUTTON_EDIT.'</a></div><br /><br />');
			$contents[] = array ('align' => 'center', 'text' => '<br />'.TEXT_EDIT_OTHER.'<br /><div align="center"><a class="button" onClick="this.blur();" href="'.xtc_href_link(FILENAME_ORDERS_EDIT, 'edit_action=other&oID='.$_GET['oID']).'">'.BUTTON_EDIT.'</a></div><br /><br />');
