...
gm_pdf_action.php 16.01.2008 pt
	Gambio OHG
	http://www.gambio.de
	Copyright (c) 2008 Gambio OHG
   Released under the GNU General Public License (Version 2)
	...
	*/
	...
	switch(($_GET['action'])) {
		...
		case 'gm_pdf_update_lang':
			...
			foreach($_POST as $key => $value) {
				if($key != session_name()) {
					$result = gm_set_content($key, strip_tags(gm_prepare_string(urldecode(utf8_decode($value)))), $lang_id);
				}
			}
		...
		case 'gm_pdf_post':
			foreach($_POST as $key => $value) {
				$result = gm_set_content($key, strip_tags(gm_prepare_string(utf8_decode($value))), $lang_id);
			}
			echo '<b style="color:#339900">' . PROCEED . '</b>';
		break;
		...
		case 'gm_pdf_content':
			...
			if($_GET['subpage'] == 'email_text') {
				...
				include(DIR_FS_ADMIN . 'gm/gm_pdf/gm_pdf_email_text.php');
				...
				include(DIR_FS_ADMIN . 'gm/gm_pdf/gm_pdf_footer.php');
				...
				$gm_values = gm_get_content($boxes, $lang_id);
				include(DIR_FS_ADMIN . 'gm/gm_pdf/gm_pdf_conditions.php');
				...
				include(DIR_FS_ADMIN . 'gm/gm_pdf/gm_pdf_order_info.php');
				...
				include(DIR_FS_ADMIN . 'gm/gm_pdf/gm_pdf_content.php');
			}
		...
		case 'gm_pdf_conf':
			...
			if($_GET['subpage'] == 'layout') {
				...
				include(DIR_FS_ADMIN . 'gm/gm_pdf/gm_pdf_layout.php');
				...
				include(DIR_FS_ADMIN . 'gm/gm_pdf/gm_pdf_invoicing.php');

			} elseif($_GET['subpage'] == 'protection') {

				$boxes = array(
								'GM_PDF_ALLOW_PRINTING',
				...
				include(DIR_FS_ADMIN . 'gm/gm_pdf/gm_pdf_protection.php');
				...
				$gm_values = gm_get_conf($boxes);
				include(DIR_FS_ADMIN . 'gm/gm_pdf/gm_pdf_display.php');
			}
		...
		case 'gm_pdf_fonts':
			...
			include(DIR_FS_ADMIN . 'gm/gm_pdf/gm_pdf_fonts.php');
		break;
		...
		case 'gm_pdf_preview':
			...
			include(DIR_FS_ADMIN . 'gm/gm_pdf/gm_pdf_preview.php');
		break;
		...
		case 'gm_box_submenu_content':
			include(DIR_FS_ADMIN . 'gm/gm_pdf/gm_pdf_submenu_content.php');
		break;

		case 'gm_box_submenu_conf':
			include(DIR_FS_ADMIN . 'gm/gm_pdf/gm_pdf_submenu_conf.php');
		break;
	}	
