...
?>
      <tr>
        <td>
			<div class="pageHeading" style="background-image:url(images/gm_icons/kunden.png)"><?php /* BOF GM_MOD */ echo htmlspecialchars_wrapper($cInfo->customers_firstname).' '.htmlspecialchars_wrapper($cInfo->customers_lastname); /* EOF GM_MOD */?></div>
        <table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="main"><?php echo HEADING_TITLE_STATUS  .': ' . $customers_statuses_array[$customers['customers_status']]['text'] ; ?>&nbsp;&nbsp;<?php if ($customers_statuses_array[$customers['customers_status']]['csa_image'] != '') { echo xtc_image(DIR_WS_ICONS . $customers_statuses_array[$customers['customers_status']]['csa_image'], ''); } ?></td>
          </tr>
        </table></td>
      </tr>
      <tr>
      ...
      <tr><?php /* BOF GM_MOD */ echo xtc_draw_form('customers', FILENAME_CUSTOMERS, xtc_get_all_get_params(array('action')) . 'action=update', 'post') . xtc_draw_hidden_field('default_address_id', $cInfo->customers_default_address_id); ?>
...
?>
          <tr>
            <td class="main" bgcolor="#3A83D0" style="color:#DDDDDD"><?php echo ENTRY_CID; ?></td>
            <td class="main" width="100%" bgcolor="#3A83D0" style="color:#DDDDDD"><?php
...
?></td>
	...
	if ($entry_firstname_error == true) {
		...
		echo xtc_draw_input_field('customers_firstname', $cInfo->customers_firstname, 'maxlength="32"', false); // BOF GM_MOD
	}
?></td>
          </tr>
          <tr>
	...
	if ($error == true) {
		if ($entry_lastname_error == true) {
			...
			echo htmlentities_wrapper($cInfo->customers_lastname) . xtc_draw_hidden_field('customers_lastname');
		}
	} else {
      ...
      <tr>
		...
		if ($error == true) {
			if ($entry_company_error == true) {
				// BOF GM_MOD:
				echo xtc_draw_input_field('entry_company', $cInfo->entry_company, 'maxlength="255"').'&nbsp;'.ENTRY_COMPANY_ERROR;
			} else {
...
?>
	...
	if ($error == true) {
		...
		echo xtc_draw_input_field('entry_street_address', $cInfo->entry_street_address, 'maxlength="64"', false); // BOF GM_MOD
	}
?></td>
          </tr>
<?php
...
?>
	...
	if ($error == true) {
		...
		echo xtc_draw_input_field('entry_postcode', $cInfo->entry_postcode, 'maxlength="8"', false); // BOF GM_MOD
	}
?></td>
          </tr>
          <tr>
	...
	if ($error == true) {
		...
		echo xtc_draw_input_field('entry_city', $cInfo->entry_city, 'maxlength="32"', false); // BOF GM_MOD
	}
?></td>
          </tr>
<?php
...
?></td>
	...
	if ($error == true) {
		...
		echo xtc_draw_input_field('customers_telephone', $cInfo->customers_telephone, 'maxlength="32"', false); // BOF GM_MOD
	}
?></td>
          </tr>
          <tr>
...
?></td>
      ...
      <tr>
        <td class="formArea"><table width= "100%" border="0" cellspacing="2" cellpadding="2">
...
?></td>
            ...
            <td class="main" bgcolor="#3A83D0" style="color:#DDDDDD"><?php echo ENTRY_NEW_PASSWORD; ?></td>
            <td class="main" bgcolor="#3A83D0" style="color:#DDDDDD"><?php
...
?></td>      
	...
	if ($processed == true) {
		// BOF GM_MOD
		if (empty($cInfo->customers_newsletter)) {
		...
		// BOF GM_MOD:
		if (empty($cInfo->customers_newsletter)) $cInfo->customers_newsletter = '0';
...
<?php include(DIR_WS_MODULES . FILENAME_CUSTOMER_MEMO); ?>
      ...
      <tr>
        <td align="right" class="main"><input type="submit" class="button float_right" onClick="this.blur();" value="<?php echo BUTTON_UPDATE; ?>"><?php echo ' <a class="button float_right" onClick="this.blur();" href="' . xtc_href_link(FILENAME_CUSTOMERS, xtc_get_all_get_params(array('action'))) .'">' . BUTTON_CANCEL . '</a>'; ?></td>
      </tr></form>
<?php
...
?>
      <tr>
		...
		<div class="pageSubHeading orders_form">
			...
			<?php 
				...
				echo HEADING_TITLE_STATUS . ' ' . xtc_draw_pull_down_menu('status',xtc_array_merge($select_data, $customers_statuses_array), '99', 'onChange="this.form.submit();"').xtc_draw_hidden_field(xtc_session_name(), xtc_session_id());
			?>
			</form>
		</div>
		<br />
      ...
      <tr>
        <td>
          ...
          ?>
		<br />
        <table style="clear: both;" border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
	...
	if (($_GET['search']) && (xtc_not_null($_GET['search']))) {
		...
		if(strlen($keywords)==1 AND !is_numeric($keywords)) $search = "and (c.customers_lastname like '".$keywords."%' or c.customers_firstname like '".$keywords."%')";
...
?></td>
			<td class="dataTableContent"><b><?php echo htmlspecialchars_wrapper($customers['customers_lastname']); ?></b></td>
...
?>
              <tr>
                <td colspan="7"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $customers_split->display_count($customers_query_numrows, '20', $_GET['page'], TEXT_DISPLAY_NUMBER_OF_CUSTOMERS); ?><?php echo "<br />" .  $customers_split->display_links($customers_query_numrows, '20', MAX_DISPLAY_PAGE_LINKS, $_GET['page'], xtc_get_all_get_params(array('page', 'info', 'x', 'y', 'cID'))); ?></td>
					<td align="right"><?php echo '<a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_CREATE_ACCOUNT) . '">' . BUTTON_CREATE_ACCOUNT . '</a>'; ?></td>
				  </tr>
<?php
...
<?php
	...
	switch ($_GET['action']) {
		case 'confirm' :
			...
			$contents[] = array ('align' => 'center', 'text' => '<br /><div align="center"><input type="submit" class="button" value="'.BUTTON_DELETE.'"><a class="button" href="'.xtc_href_link(FILENAME_CUSTOMERS, xtc_get_all_get_params(array ('cID', 'action')).'cID='.$cInfo->customers_id).'">'.BUTTON_CANCEL.'</a></div>');
			break;
		...
		case 'editstatus' :
			if ($_GET['cID'] != 1) {
				...
				$contents[] = array ('text' => '<table nowrap border="0" cellspacing="0" cellpadding="0"><tr><td style=";" nowrap class="smallText" align="center"><b>'.TABLE_HEADING_NEW_VALUE.' </b></td><td style="" nowrap class="smallText" align="center"><b>'.TABLE_HEADING_DATE_ADDED.'</b></td></tr>');
				...
				$contents[] = array ('align' => 'center', 'text' => '<br /><div align="center"><input type="submit" class="button" value="'.BUTTON_UPDATE.'"><a class="button" href="'.xtc_href_link(FILENAME_CUSTOMERS, xtc_get_all_get_params(array ('cID', 'action')).'cID='.$cInfo->customers_id).'">'.BUTTON_CANCEL.'</a></div>');
		...
		default :
			...
			if ($_GET['action'] == 'iplog') {
				if (isset ($_GET['cID'])) {
					$contents[] = array ('text' => '<br /><b>IP-Log:'); // BOF GM_MOD EOF//
					$customers_id = xtc_db_prepare_input($_GET['cID']);
