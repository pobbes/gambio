  ...
  if ($_GET['action']) {
    switch ($_GET['action']) {
      ...
      case 'backupnow':
        ...
        while ($tables = xtc_db_fetch_array($tables_query)) {
          ...
          // BOF GM_MOD
            ...
            if (strlen($fields['Default']) > 0){
				if($fields['Default'] != 'CURRENT_TIMESTAMP') $schema .= ' default \'' . $fields['Default'] . '\'';
            	else $schema .= ' not null default CURRENT_TIMESTAMP on update CURRENT_TIMESTAMP';
            }
            if ($fields['Null'] != 'YES' && $fields['Default'] != 'CURRENT_TIMESTAMP') $schema .= ' not null';
          ...
          while ($keys = xtc_db_fetch_array($keys_query)) {
			...
			//NC_MOD:
			if($keys['Index_type'] == "FULLTEXT") continue;
          ...
          while ($rows = xtc_db_fetch_array($rows_query)) {
           ...
           while (list(,$i) = each($table_list)) {
              if (!isset($rows[$i])) {
                ...
                $gm_row_values[] = "'".addslashes($rows[$i])."'";
              } else {
                $gm_row_values[] = "''";
              }
            }
            
            $schema_insert .= implode(", ", $gm_row_values) . ');' . "\n";
		...
		{
			if(version_compare($t_result_array['version'], '4.1') >= 0)
			{
				$schema .= "\nALTER TABLE `content_manager` CHANGE `gm_url_keywords` `gm_url_keywords` VARCHAR( 255 ) CHARACTER SET latin1 COLLATE latin1_general_cs NOT NULL;\n
ALTER TABLE `categories_description` CHANGE `gm_url_keywords` `gm_url_keywords` VARCHAR( 255 ) CHARACTER SET latin1 COLLATE latin1_general_cs NOT NULL;\n
ALTER TABLE `products_description` CHANGE `gm_url_keywords` `gm_url_keywords` VARCHAR( 255 ) CHARACTER SET latin1 COLLATE latin1_general_cs NOT NULL;\n";
			}
		}
...
// join the query before the comment appeared, with the rest of the dump
              ...
              if ( strpos(strtolower($next), 'create') !== false ||
					strpos(strtolower($next), 'insert') !== false ||
					strpos(strtolower($next), 'drop t') !== false  ||
					strpos(strtolower($next), 'alter t') !== false )
			  {
                $next = '';
  ...
  if ($dir_ok) {
    ...
    for ($files = 0, $count = sizeof($contents); $files < $count; $files++) {
      ...
      if (((!$_GET['file']) || ($_GET['file'] == $entry)) && (!$buInfo) && ($_GET['action'] != 'backup') && ($_GET['action'] != 'restorelocal')) {
        ...
        $file_array['size'] = number_format((double)filesize(DIR_FS_BACKUP . $entry)) . ' bytes';
...
?>
                ...
                <td class="dataTableContent" align="right" onclick="document.location.href='<?php echo xtc_href_link(FILENAME_BACKUP, $onclick_link); ?>'"><?php echo number_format((double)filesize(DIR_FS_BACKUP . $entry)); ?> bytes</td>
...
?>
              ...
              <tr>
                <td colspan="4" align="right" class="smallText"><?php if ( ($_GET['action'] != 'backup') && ($dir) ) echo '<a style="float:right" class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_BACKUP, 'action=backup') . '">' . BUTTON_BACKUP . '</a>'; if ( ($_GET['action'] != 'restorelocal') && ($dir) ) echo '<a style="float:right"  class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_BACKUP, 'action=restorelocal') . '">' . BUTTON_RESTORE . '</a>'; ?></td>
              </tr>
<?php
  ...
  switch ($_GET['action']) {
    case 'backup':
      ...
      if ($messageStack->size > 0) {
        ...
        // BOF GM_MOD:
				$contents[] = array('text' => '<br />' . xtc_draw_radio_field('compress', 'gzip') . ' ' . TEXT_INFO_USE_GZIP);
        $contents[] = array('text' => xtc_draw_radio_field('compress', 'zip') . ' ' . TEXT_INFO_USE_ZIP);
        // BOF GM_MOD:
				$contents[] = array('text' => xtc_draw_radio_field('compress', 'no', true) . ' ' . TEXT_INFO_USE_NO_COMPRESSION);
        $contents[] = array('text' => '<br />' . xtc_draw_checkbox_field('download', 'yes') . ' ' . TEXT_INFO_DOWNLOAD_ONLY . '*<br /><br />*' . TEXT_INFO_BEST_THROUGH_HTTPS);
      }

      $contents[] = array('align' => 'center', 'text' => '<br /><div align="center"><input type="submit" class="button" onClick="this.blur();" value="' . BUTTON_BACKUP . '"/><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_BACKUP) . '">' . BUTTON_CANCEL . '</a></div>');
      break;
    case 'restore':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br /><div align="center"><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_BACKUP, 'file=' . $buInfo->file . '&action=restorenow') . '">' . BUTTON_RESTORE . '</a><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_BACKUP, 'file=' . $buInfo->file) . '">' . BUTTON_CANCEL . '</a></div>');
      break;
    case 'restorelocal':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br /><div align="center"><input type="submit" class="button" onClick="this.blur();" value="' . BUTTON_RESTORE . '"/><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_BACKUP) . '">' . BUTTON_CANCEL . '</a></div>');
      break;
    case 'delete':
      $heading[] = array('text' => '<b>' . $buInfo->date . '</b>');
      ...
      $contents[] = array('align' => 'center', 'text' => '<br /><div align="center"><input type="submit" class="button" onClick="this.blur();" value="' . BUTTON_DELETE . '"/><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_BACKUP, 'file=' . $buInfo->file) . '">' . BUTTON_CANCEL . '</a></div>');
      break;
    default:
      if (is_object($buInfo)) {
        $heading[] = array('text' => '<b>' . $buInfo->date . '</b>');
		$contents[] = array ('align' => 'center', 'text' => '<div style="padding-top: 5px; font-weight: bold; ">' . TEXT_MARKED_ELEMENTS . '</div><br />');           
        $contents[] = array('align' => 'center', 'text' => '<div align="center"><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_BACKUP, 'file=' . $buInfo->file . '&action=restore') . '">' . BUTTON_RESTORE . '</a> <a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_BACKUP, 'file=' . $buInfo->file . '&action=delete') . '">' . BUTTON_DELETE . '</a></div>');
