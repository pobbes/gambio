<?php
/* --------------------------------------------------------------
   manufacturer_info.php 2008-08-09 gambio
   Gambio OHG
   http://www.gambio.de
...
$box_content='';
  ...
  $box_manufacturers_info= $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_manufacturers_info.html',$cache_id);
    ...
    if ($manufacturer['manufacturers_name']!='' || $_SESSION['style_edit_mode'] == 'edit') {

			$gm_box_pos = $coo_template_control->get_menubox_position('manufacturers_info');

	$smarty->assign($gm_box_pos, $box_manufacturers_info);
}
?>