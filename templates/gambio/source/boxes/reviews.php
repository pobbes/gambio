...
$box_content='';
  ...
  if ($random_product) {
    ...
    $box_content = '<a href="' . xtc_href_link(FILENAME_PRODUCT_REVIEWS_WRITE, xtc_product_link($product->data['products_id'],$product->data['products_name'])) . '">' . xtc_image('templates/' . CURRENT_TEMPLATE . '/img/box_write_review.gif', IMAGE_BUTTON_WRITE_REVIEW) . '</a><br />
					<a href="' . xtc_href_link(FILENAME_PRODUCT_REVIEWS_WRITE, xtc_product_link($product->data['products_id'],$product->data['products_name'])) . '">' . BOX_REVIEWS_WRITE_REVIEW .'</a>';
   }

  if ($box_content!='' || $_SESSION['style_edit_mode'] == 'edit') {
 ...
 if (!CacheCheck()) {
  ...
  $smarty->assign($gm_box_pos,$box_reviews);
