<?php
/* --------------------------------------------------------------
   manufacturers.php 2012-06-26 gm
   Gambio GmbH
   http://www.gambio.de
...
if (!$box_smarty->is_cached(CURRENT_TEMPLATE.'/boxes/box_manufacturers.html', $cache_id) || !$cache) {
	...
	if (xtc_db_num_rows($manufacturers_query, true) <= MAX_DISPLAY_MANUFACTURERS_IN_A_LIST) {
		...
		// BOF GM_MOD:
		$box_content = xtc_draw_form('manufacturers', xtc_href_link(FILENAME_DEFAULT, '', 'NONSSL', false, true, true), 'get').xtc_draw_pull_down_menu('manufacturers_id', $manufacturers_array, (int)$_GET['manufacturers_id'], 'onchange="this.form.submit();" size="'.MAX_MANUFACTURERS_LIST.'" style="width: 100%" class="lightbox_visibility_hidden"').xtc_hide_session_id().'</form>';
...
if (!$cache) {
	...
	$smarty->assign($gm_box_pos, $box_manufacturers);
?>