...
require_once (DIR_FS_INC.'xtc_random_select.inc.php');
        ...
        if (is_object($product) && $product->isProduct()) {
            $recomendedObjects = recommend(getTopSellingStrategy(), $product->pID, getBoxAlsoClickedMaxDisplay() * 2);
...
if ($builded || $_SESSION['style_edit_mode'] == 'edit') {
	...
	$box_smarty->assign('box_header', gm_get_content(YOOCHOOSE_BOX_TOP_SELLING_HEADER, $_SESSION['languages_id']));
	
	// set cache ID
	 if (!CacheCheck()) {
		$box_smarty->caching = 0;
		$box_specials = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_yoochoose_percent_items.html');
	} else {
		...
		$box_specials = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_yoochoose_percent_items.html', $cache_id);
	}
  ...
  //GM_MOD:
  	if(is_dir(DIR_FS_CATALOG.'StyleEdit/') === false)
		...
		$gm_box_pos = $gmBoxesMaster->get_position('yoochoose_top_selling');
