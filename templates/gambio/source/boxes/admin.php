...
$box_smarty->assign('tpl_path','templates/'.CURRENT_TEMPLATE.'/');
  ...
  $orders_contents .= '<a href="#" onclick="window.open('.$t_url.'); return false;">' . TEXT_VALIDATING . '</a>: ' . $orders_status_validating . '<br />';
  ...
  while ($orders_status = xtc_db_fetch_array($orders_status_query)) {
	...
	$t_url = "'".xtc_href_link_admin(FILENAME_ORDERS, 'selected_box=customers&status=' . $orders_status['orders_status_id'], 'NONSSL')."'";
    $orders_contents .= '<a href="#" onclick="window.location.href='.$t_url.'; return false;">' . $orders_status['orders_status_name'] . '</a>: ' . $orders_pending['count'] . '<br />';
  }
  ...
  $t_url = "'".xtc_href_link_admin(FILENAME_START,'', 'NONSSL')."'";
  $admin_image = '<a href="#" onclick="window.location.href='.$t_url.'; return false;">'.xtc_image_button('button_admin.gif', IMAGE_BUTTON_ADMIN).'</a>';
   if ($product->isProduct()) {
    // BOF GM_MOD:
		$t_url = "'". xtc_href_link_admin(FILENAME_EDIT_PRODUCTS, 'cPath=' . $cPath . '&pID=' . $product->data['products_id'], 'NONSSL') . '&action=new_product'. "'";
		$admin_link='<a href="#" onclick="window.location.href='.$t_url.'; return false;">' . xtc_image_button('edit_product.gif', IMAGE_BUTTON_PRODUCT_EDIT, 'style="margin-top:4px"') . '</a>';
  }
  ...
  $box_content= '<b>' . BOX_TITLE_STATISTICS . '</b><br />' . $orders_contents . '<br />' .
    ...
    $smarty->assign($gm_box_pos,$box_admin);
