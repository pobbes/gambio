<?php
/* --------------------------------------------------------------
   ScrollerContentView.inc.php 2010-09-22 gambio
   Gambio GmbH
   http://www.gambio.de
...
class ScrollerContentView extends ContentView
{
	function ScrollerContentView()
	{
		$this->set_content_template('boxes/box_gm_scroller.html');
	}

	function get_html($p_languages_id = '2')
	{
		$c_languages_id = (int)$p_languages_id;
		$this->set_content_data('CONTENT', gm_get_content('GM_SCROLLER_CONTENT', $c_languages_id));
		$this->set_content_data('HEIGHT', gm_get_conf('GM_SCROLLER_HEIGHT'));
