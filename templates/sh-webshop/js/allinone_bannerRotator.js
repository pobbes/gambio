(function($) {
		...
		function animate_texts(current_obj,bannerRotator_the,bannerControls) {
			...
			jQuery(current_obj.currentImg.attr('data-text-id')).css('width',bannerRotator_the.width()+'px');
			jQuery(current_obj.currentImg.attr('data-text-id')).css('left',bannerControls.css('left'));//alert (bannerRotator_the.width());
        ...
        function bannerRotator_generate_stripes(bannerRotator_container,options,current_obj) {
			jQuery('.stripe', bannerRotator_container).remove();
			jQuery('.block', bannerRotator_container).remove();
            stripe_width = Math.round(bannerRotator_container.width()/options.numberOfStripes);
			new_stripe_width=stripe_width;
			//alert(bannerRotator_container.width()+' - '+stripe_width);
        	for(var i = 0; i < options.numberOfStripes; i++){
				if (i == options.numberOfStripes-1) {
					new_stripe_width=bannerRotator_container.width()-stripe_width*(options.numberOfStripes-1);
					//alert (stripe_width+'  -  '+new_stripe_width);
				}
				bannerRotator_container.append(
        ...
        // generate the blocks
        function bannerRotator_generate_blocks(bannerRotator_container,options,current_obj) {
			jQuery('.stripe', bannerRotator_container).remove();
			jQuery('.block', bannerRotator_container).remove();
			var block_width = Math.round(bannerRotator_container.width()/options.numberOfColumns);
			var block_height = Math.round(bannerRotator_container.height()/options.numberOfRows);	        	
		...
		function animate_block(block,i,j,options,bannerRotator_container){
            var w = block.width();
            var h = block.height();
            block.css({'width':'0'});
            block.css({'height':'0'});
            if (i==options.numberOfRows-1 && j==options.numberOfColumns-1) 
            	setTimeout(function(){
					block.animate({ opacity:'1.0', width:w, height:h }, (options.effectDuration*1000)/3 , '', function(){ bannerRotator_container.trigger('effectComplete'); });
				}, (delay_time));               	
            else	
				setTimeout(function(){
        ...
        // navigation
		function bannerRotator_navigation(direction,options,current_obj,total_images,current_effect,bannerRotator_container,bottomNavButs,imgs){
			...
			if (navigateAllowed) {
				...
				if(current_effect == 'fade' || current_effect == 'slideFromLeft' || current_effect == 'slideFromRight' || current_effect == 'slideFromTop' || current_effect == 'slideFromBottom'){
					//alert ("fade");
					bannerRotator_generate_stripes(bannerRotator_container,options,current_obj);
					var first_stripe = jQuery('.stripe:first', bannerRotator_container);
					
					if (current_effect == 'fade') {
						first_stripe.css({
		                    'height': '100%',
		                    'width': bannerRotator_container.width() + 'px'
		                });
						first_stripe.animate({ opacity:'1.0' }, (options.effectDuration*2000), '', function(){ bannerRotator_container.trigger('effectComplete'); });
					}
					...
					if (current_effect == 'slideFromLeft') {
						...
						first_stripe.animate({ opacity:'1.0', width:bannerRotator_container.width() + 'px' }, (options.effectDuration*1000), '', function(){ bannerRotator_container.trigger('effectComplete'); });
					}
					...
					if (current_effect == 'slideFromRight') {
						first_stripe.css({
		                    'height': '100%',
		                    'width':  '0',
		                    'left': bannerRotator_container.width()+5 + 'px'
		                });
						first_stripe.animate({ opacity:'1.0', left:'0', 'width':  bannerRotator_container.width() + 'px' }, (options.effectDuration*1000), '', function(){ bannerRotator_container.trigger('effectComplete'); });
					}
					...
					if (current_effect == 'slideFromTop') {
						...
						first_stripe.animate({ opacity:'1.0', height:bannerRotator_container.height() + 'px' }, (options.effectDuration*1000), '', function(){ bannerRotator_container.trigger('effectComplete'); });
					}
					...
					if (current_effect == 'slideFromBottom') {
						first_stripe.css({
		                    'height': '0',
		                    'width': bannerRotator_container.width() + 'px',
		                    'top': bannerRotator_container.height() + 'px'
		                });
						first_stripe.animate({ opacity:'1.0', top:0, height:bannerRotator_container.height() + 'px' }, (options.effectDuration*1000), '', function(){ bannerRotator_container.trigger('effectComplete'); });
					}					
				...
				if(current_effect.indexOf('Stripes')>=0) {
					...
					if (current_effect.indexOf('Reverse')>=0){
						var stripes = jQuery('.stripe', bannerRotator_container).myReverse();
					} else {
						var stripes = jQuery('.stripe', bannerRotator_container);
					}
					delay_time = 100;
					i = 0;
					stripes.each(function(){
						...
						if(current_effect=='leftRightFadingStripes' || current_effect=='leftRightFadingReverseStripes') {
							...
							if(i == options.numberOfStripes-1){
								setTimeout(function(){
									stripe.animate({ width:aux_stripe_width+'px', opacity:'1.0' }, (options.effectDuration*800), '', function(){ bannerRotator_container.trigger('effectComplete'); });
								}, (delay_time));
							} else {
								setTimeout(function(){
								...
								if(i == options.numberOfStripes-1){
									setTimeout(function(){
										stripe.animate({ height:'100%', opacity:'1.0' }, (options.effectDuration*1000), '', function(){ bannerRotator_container.trigger('effectComplete'); });
									}, (delay_time));
								} else {
									setTimeout(function(){
				...
				if(current_effect.indexOf('Blocks')>=0) {
					bannerRotator_generate_blocks(bannerRotator_container,options,current_obj);
					if (current_effect.indexOf('Reverse')>=0){
						var blocks = jQuery('.block', bannerRotator_container).myReverse();
					} else if (current_effect=='randomBlocks') {
						var blocks = shuffle(jQuery('.block', bannerRotator_container));
					} else {
						var blocks = jQuery('.block', bannerRotator_container);
					}
					delay_time = 100;
					...
					if (current_effect=='randomBlocks') {
						...
						blocks.each(function(){
							...
							animate_block(block,0,0,options,bannerRotator_container);
							while (row_i<options.numberOfRows-1 || col_i<options.numberOfColumns-1) {
								...
								while (i>=0 && j<=col_i) {
									...
									animate_block(block,i,j,options,bannerRotator_container);
									//alert (i+' - '+j);
								}
	...
	$.fn.bannerRotator = function(options) {
		...
		var options = $.extend({},$.fn.bannerRotator.defaults, options);

		return this.each(function() {
			var bannerRotator_the = jQuery(this);
			bannerRotator_the.css("display","block");
			
			//the controllers
			var bannerRotator_wrap = jQuery('<div></div>').addClass('bannerRotator').addClass(options.skin);
			var bannerControls = jQuery('<div class="bannerControls">   <div class="leftNav"></div>   <div class="rightNav"></div>      </div>');
			bannerRotator_the.wrap(bannerRotator_wrap);
			bannerRotator_the.after(bannerControls);
			

			
			//the elements
			var bannerRotator_container = bannerRotator_the.parent('.bannerRotator');
			var bannerControls = jQuery('.bannerControls', bannerRotator_container);
			...
			bannerRotator_the.after(bottomNavLeft_aux);
			bannerRotator_the.after(bottomNav_aux);
			bannerRotator_the.after(bottomNavRight_aux);
			 
			if (!options.showAllControllers)
				bannerControls.css("display","none");			
			
			
			var bannerRotator_leftNav = jQuery('.leftNav', bannerRotator_container);
			var bannerRotator_rightNav = jQuery('.rightNav', bannerRotator_container);
			bannerRotator_leftNav.css("display","none");
			bannerRotator_rightNav.css("display","none");			
			if (options.showNavArrows) {
				if (options.showOnInitNavArrows) {
					bannerRotator_leftNav.css("display","block");
					bannerRotator_rightNav.css("display","block");
				}
			}
			
			var bannerRotator_bottomNav = jQuery('.bottomNav', bannerRotator_container);
			var bannerRotator_bottomNavLeft = jQuery('.bottomNavLeft', bannerRotator_container);
			var bannerRotator_bottomNavRight = jQuery('.bottomNavRight', bannerRotator_container);
			var bannerRotator_bottomOverThumb;
			bannerRotator_bottomNav.css("display","block");
			bannerRotator_bottomNavLeft.css("display","block");
			bannerRotator_bottomNavRight.css("display","block");
			if (!options.showBottomNav) {
				bannerRotator_bottomNav.css("display","none");
				bannerRotator_bottomNavLeft.css("display","none");
				bannerRotator_bottomNavRight.css("display","none");
			}
			if (!options.showOnInitBottomNav) {
				bannerRotator_bottomNav.css("left","-5000px");
				bannerRotator_bottomNavLeft.css("left","-5000px");
				bannerRotator_bottomNavRight.css("left","-5000px");				
			}
			...
			//set banner size
			bannerRotator_container.width(options.width);
			bannerRotator_container.height(options.height);
			
			bannerControls.width('100%');
			bannerControls.height('100%');
			
			//get images
			var imgs = bannerRotator_the.children();
			var bottomNavBut;
			...
			imgs.each(function() {
	            ...
	            if(current_obj.currentImg.is('img')){
		            ...
		            //generate bottomNav
		            bottomNavBut = jQuery('<div class="bottomNavButtonOFF left" rel="'+ (total_images-1) +'"></div>');
		            bannerRotator_bottomNav.append(bottomNavBut);
		            ...
		            bottomNavMarginTop=parseInt((bannerRotator_bottomNav.height()-parseInt(bottomNavBut.css('height').substring(0, bottomNavBut.css('height').length-2)))/2);
			...
			bannerRotator_bottomNav.width(bottomNavWidth);
			if (options.showOnInitBottomNav) {
				bannerRotator_bottomNav.css("left",parseInt((bannerRotator_container.width()-bottomNavWidth)/2)+'px');
				bannerRotator_bottomNavLeft.css("left",parseInt(bannerRotator_bottomNav.css('left').substring(0, bannerRotator_bottomNav.css('left').length-2))-bannerRotator_bottomNavLeft.width()+'px');
				bannerRotator_bottomNavRight.css("left",parseInt(bannerRotator_bottomNav.css('left').substring(0, bannerRotator_bottomNav.css('left').length-2))+bannerRotator_bottomNav.width()+parseInt(bottomNavBut.css('padding-left').substring(0, bottomNavBut.css('padding-left').length-2))+'px');
			}
	        ...
	        bannerRotator_container.css('background','url("'+ current_obj.currentImg.attr('src') +'") no-repeat');
			...
			if (options.enableTouchScreen) {
				...
				bannerRotator_container.wrap('<div id="bannerRotatorParent_'+randomNo+'" style="position:relative;" />');
				...
				bannerRotator_container.css('cursor','url(templates/sh-webshop/images/hand.cur),url(templates/sh-webshop/images/hand.cur),move');
				bannerRotator_container.css('left','0px');
				bannerRotator_container.css('position','absolute');

				rightVal=parseInt(bannerRotator_rightNav.css('right').substring(0, bannerRotator_rightNav.css('right').length-2));
			
				//alert(bannerRotator_container.parent().attr('id'));
				
				//jQuery("body").css("overflow", "hidden");
				
				bannerRotator_container.mousedown(function() {
					rightVal=parseInt(bannerRotator_rightNav.css('right').substring(0, bannerRotator_rightNav.css('right').length-2));					
					if (rightVal<0 && !arrowClicked) {
						bannerRotator_rightNav.css('visibility','hidden');
						bannerRotator_leftNav.css('visibility','hidden');
						bannerRotator_rightNav.css('right','0');
					}
				});	
				bannerRotator_container.mouseup(function() {
					arrowClicked=false;
					if (rightVal<0) {	
						bannerRotator_rightNav.css('right',rightVal+'px');
						bannerRotator_rightNav.css('visibility','visible');
						bannerRotator_leftNav.css('visibility','visible');							
					}
				});					
				
				bannerRotator_container.draggable({ 
					axis: 'x',
					containment: 'parent',
					//scroll:false,
					//revert:true,
					...
					stop: function(event, ui) {
						if (!current_obj.effectIsRunning) {
							...
							bannerRotator_navigation(direction,options,current_obj,total_images,current_effect,bannerRotator_container,bottomNavButs,imgs);
						}
						if (rightVal<0) {	
							bannerRotator_rightNav.css('right',rightVal+'px');
							bannerRotator_rightNav.css('visibility','visible');
			...
			animate_texts(current_obj,bannerRotator_the,bannerControls);
	        
			

	        //Event when Animation finishes
			bannerRotator_container.bind('effectComplete', function(){
				...
				if (options.autoPlay>0 && total_images>1 && !mouseOverBanner) {
					...
					timeoutID=setTimeout(function(){ bannerRotator_navigation(1,options,current_obj,total_images,current_effect,bannerRotator_container,bottomNavButs,imgs)},options.autoPlay*1000);
				}				
	        }); //bind
			...
			//pause on hover
			bannerRotator_container.mouseenter(function() {
			...
			bannerRotator_container.mouseleave(function() {
			...
			/*//a href
			bannerRotator_container.click(function() {
				alert("a");
			...
			//controllers
			bannerRotator_leftNav.mousedown(function() {
				arrowClicked=true;
				if (!current_obj.effectIsRunning) {
					//mouseOverBanner=false;
					clearTimeout(timeoutID);
					bannerRotator_navigation(-1,options,current_obj,total_images,current_effect,bannerRotator_container,bottomNavButs,imgs);
				}
			});
			bannerRotator_leftNav.mouseup(function() {
				arrowClicked=false;
			});				
			
			bannerRotator_rightNav.mousedown(function() {
				arrowClicked=true;
				if (!current_obj.effectIsRunning) {
					//mouseOverBanner=false;
					clearTimeout(timeoutID);
			...
			bannerRotator_rightNav.mouseup(function() {
				arrowClicked=false;
			});				
			...
			//bottom nav
			var bottomNavButs=jQuery(".bottomNavButtonOFF", bannerRotator_container);
			...
			bottomNavButs.mouseenter(function() {
				...
				if (options.showPreviewThumbs) {
					bannerRotator_bottomOverThumb = jQuery('<div class="bottomOverThumb"></div>');
					currentBut.append(bannerRotator_bottomOverThumb);
					...
					bannerRotator_bottomOverThumb.html('<img src="'+ options.thumbsFolder + image_name + '">');
				}
			...
			bottomNavButs.mouseleave(function() {
				...
				if (options.showPreviewThumbs) {
					bannerRotator_bottomOverThumb.remove();
				}				
	...
	$.fn.bannerRotator.defaults = {
			...
			thumbsFolder: 'images/slider/thumbs',
