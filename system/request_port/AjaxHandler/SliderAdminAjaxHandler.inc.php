<?php
/* --------------------------------------------------------------
   SliderAdminAjaxHandler.inc.php 2012-12-14 gambio
   Gambio GmbH
   http://www.gambio.de
...
class SliderAdminAjaxHandler extends AjaxHandler
{
	...
	{
		if(!check_data_type($p_slider_image_id, 'int', false, E_USER_ERROR)) die();
	...
	{
		if(!check_data_type($p_slider_image_area_id, 'int', false, E_USER_ERROR)) die();
		$c_slider_image_area_id = (int)$p_slider_image_area_id;
		
		if(!check_data_type($p_shape, 'string', false, E_USER_ERROR)) die();
		$c_shape = $p_shape;

		if(!check_data_type($p_coords, 'string', false, E_USER_ERROR)) die();
		$c_coords = $p_coords;
		
		if(!check_data_type($p_title, 'string', false, E_USER_ERROR)) die();
		$c_title = utf8_decode( stripslashes( $p_title ) );
		
		if(!check_data_type($p_link_url, 'string', false, E_USER_ERROR)) die();
		$c_link_url = utf8_decode( $p_link_url );

		if(!check_data_type($p_link_target, 'string', false, E_USER_ERROR)) die();
		$c_link_target = $p_link_target;
    
    if(!check_data_type($p_flyover_content, 'string', false, E_USER_ERROR)) die();
	...
	function get_image_area_data($p_slider_image_id)
	{
		...
		for($i = 0; $i < count($coo_slider_image_area_array); $i++)
		{
			$t_item_array = array(
									...
									'title' => utf8_encode( $coo_slider_image_area_array[$i]->get_title() ),
									'link_url' => utf8_encode( $coo_slider_image_area_array[$i]->get_link_url() ),
