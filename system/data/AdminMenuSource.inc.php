<?php
/* --------------------------------------------------------------
   AdminMenuSource.inc.php 2012-02-15 gm
   Gambio GmbH
   http://www.gambio.de
...
class AdminMenuSource extends MenuSourceInterface
{
	...
	function AdminMenuSource()
	{
		...
		$this->v_system_xml_path = DIR_FS_CATALOG.'system/conf/AdminMenu/';
		// user xml directory
		$this->v_user_xml_path = DIR_FS_CATALOG.'user_classes/conf/AdminMenu/';
	}
	...
	function get_groups_array( )
	{
		...
		foreach($this->v_menu_structure_array as $key => $t_value){
			$t_group = array("id" => $t_value["id"], "title" => $t_value["title"], "background" => $t_value["background"]);
