<?php
/* --------------------------------------------------------------
   DataCache.inc.php 2012-06-28 gm
   Gambio GmbH
   http://www.gambio.de
...
class DataCache
{
	...
	*/
	...
	public function set_data($p_key, $p_value, $p_persistent=false, $p_persistence_tags_array=false)
	{
		...
		if(is_object($GLOBALS['coo_debugger'])) $GLOBALS['coo_debugger']->log("new content with key ". $p_key, 'DataCache');

		if($p_persistent)
		{
			$this->write_persistent_data($p_key, $p_value);
			if(is_object($GLOBALS['coo_debugger'])) $GLOBALS['coo_debugger']->log("new persistent content with key ". $p_key, 'DataCache');
	...
	public function get_data($p_key, $p_persistent=false)
	{
		...
		if($this->key_exists($p_key, $p_persistent) == false)
		{
			...
			if(is_object($GLOBALS['coo_debugger'])) $GLOBALS['coo_debugger']->log("cached content returned with key ". $p_key, 'DataCache');
		}
		return $t_output;
	}
