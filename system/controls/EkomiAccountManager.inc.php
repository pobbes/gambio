<?php
/* --------------------------------------------------------------
   EkomiAccountManager.inc.php 2011-12-20 gm
   Gambio GmbH
   http://www.gambio.de
			...
			if(is_string($t_result))
			{
				...
				if(check_data_type($coo_result, 'object'))
				{
					if(isset($coo_result->interface_id) && isset($coo_result->interface_pw))
					{
						...
						{
							$coo_ekomi_log = new FileLog('ekomi_errors');
							$coo_ekomi_log->write(date('Y-m-d H:i:s') . " e-mail '" . EKOMI_ACCOUNT_MAIL_SUBJECT . "' could not be sent to '" . $coo_result->login_mail . "'\n");
						}
					}
				}
			}
