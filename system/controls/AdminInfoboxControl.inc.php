<?php
/* --------------------------------------------------------------
   AdminInfoboxControl.inc.php 2012-08-17 gm
   Gambio GmbH
   http://www.gambio.de
...
class AdminInfoboxControl
{
	...
	function reactivate_messages()
	{
		...
		$t_sql = "UPDATE infobox_messages SET status = 'new', date_modified = '" . date('Y-m-d H:s:i') . "' WHERE ((date_added < '" . $t_limit . "' AND date_modified = '0000-00-00 00:00:00') || (date_modified < '" . $t_limit . "' AND date_modified != '0000-00-00 00:00:00')) AND status = 'hidden'";
		$t_result = xtc_db_query($t_sql);
