<?php
/* --------------------------------------------------------------
   IndexFeatureProductFinder.inc.php 2012-06-11 gm
   Gambio GmbH
   http://www.gambio.de
...
class IndexFeatureProductFinder
{
		...
		$t_output_sql = '
						...
						FROM
							'.TABLE_PRODUCTS.' AS p
								...
								LEFT JOIN feature_index AS fi ON (fi.products_id = p.products_id) '.
								$t_sql_price_parts_array['FROM'].'
						WHERE
							p.products_status = 1
							AND pd.language_id = "'.(int)$_SESSION['languages_id'].'"
							AND fi.categories_index LIKE "'.$t_categories_search.'"
	...
	function get_feature_index_search_string($p_feature_value_group_array)
	{
		...
		if(is_object($GLOBALS['coo_debugger'])) $GLOBALS['coo_debugger']->log('get_index_search_string() $p_feature_value_group_array: '. print_r($p_feature_value_group_array, true), 'IndexFeatureProductFinder');
...
} // end of IndexFeatureProductFinder
?>