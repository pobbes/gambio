<?php
/* --------------------------------------------------------------
   FeatureValue.php 2010-12-09 gambio
   Gambio GmbH
   http://www.gambio.de
...
class FeatureValue
{
  ...
  var $v_parent_feature_id_array = array();
  ...
  function load_data_object($p_coo_data_object)
  {
    ...
    # load parent ids
    $coo_data_object_group = MainFactory::create_object('GMDataObjectGroup', array('feature_parent', array('feature_value_id' => $t_feature_value_id)));
    $t_data_object_array = $coo_data_object_group->get_data_objects_array();
    foreach($t_data_object_array as $t_data_object_item) {
      $t_feature_parent_id = $t_data_object_item->get_data_value('parent_feature_value_id');
      $this->set_feature_parent_id($t_feature_parent_id);
    }
    $coo_data_object_group = NULL;
  ...
  /*
   * set_feature_parent_id
   * @param int $p_feature_parent_id  feature_parent_id
   * @return bool
   */
  function set_feature_parent_id($p_feature_parent_id)
  {
    $this->v_parent_feature_id_array[] = (int) $p_feature_parent_id;
    return true;
  }
  ...
  /*
   * add_parent_feature_id
   * @param int $p_parent_feature_id  parent feature id
   * @return bool true:ok | false:error
   */
  function add_parent_feature_value_id($p_parent_feature_id)
  {
    array_push($this->v_parent_feature_id_array, (int) $p_parent_feature_id);
    return true;
  }

  /*
   * remove_parent_feature_id
   * @param int $p_parent_feature_id  parent feature id
   * @return bool true:ok | false:error
   */
  function remove_parent_feature_value_id($p_parent_feature_id)
  {
    $c_parent_feature_id = (int) $p_parent_feature_id;
    $t_key = array_search($c_parent_feature_id, $this->v_parent_feature_id_array);
    unset($this->v_parent_feature_id_array[$t_key]);
    return true;
  }
  ...
  function save()
  {
    ...
    foreach ($this->v_feature_value_text_array as $t_language_id => $t_feature_value_name) {
      ...
      $coo_feature_value_desc->set_data_value('feature_value_text', $t_feature_value_name);
    ...
    # save parent ids
    if (!empty($this->v_parent_feature_id_array)) {
      $coo_data_object_group = MainFactory::create_object('GMDataObject', array('feature_parent'));
      $coo_data_object_group->set_keys(array('feature_value_id' => $t_feature_value_id));
      $coo_data_object_group->set_data_value('feature_value_id', $t_feature_value_id);
      $coo_data_object_group->delete();
      $coo_data_object_group = NULL;

      $coo_feature_parent = new GMDataObject('feature_parent');
      foreach ($this->v_parent_feature_id_array as $t_key => $t_parent_id) {
        $coo_feature_parent->set_keys(array('feature_value_id' => false));
        $coo_feature_parent->set_data_value('feature_value_id', $t_feature_value_id);
        $coo_feature_parent->set_data_value('parent_feature_value_id', $t_parent_id);
        $coo_feature_parent->save_body_data();
      }
      $coo_feature_parent = NULL;
    }
  ...
  function load($p_feature_value_id)
  {
    ...
    sort($this->v_parent_feature_id_array);
  ...
  function delete()
  {
    $coo_feature_value = MainFactory::create_object('GMDataObject', array('feature_value'));
    $coo_feature_value->set_keys(array('feature_value_id' => $this->v_feature_value_id));
    $coo_feature_value->delete();
    $coo_feature_value = NULL;

    $coo_feature_value_desc = MainFactory::create_object('GMDataObject', array('feature_value_description'));
    $coo_feature_value_desc->set_keys(array('feature_value_id' => $this->v_feature_value_id));
    $coo_feature_value_desc->delete();
    $coo_feature_value_desc = NULL;
  ...
  function reset()
  {
    ...
    $this->v_parent_feature_id_array = array();
    return true;
  }
}
?>