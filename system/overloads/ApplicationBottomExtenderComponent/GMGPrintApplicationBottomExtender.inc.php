<?php
/* --------------------------------------------------------------
   GMGPrintApplicationBottomExtender.inc.php 2012-02-01 gm
   Gambio GmbH
   http://www.gambio.de
		...
		if(gm_get_conf('GM_SHOP_OFFLINE') != 'checked' || $_SESSION['customers_status']['customers_status_id'] == 0)
		{
			...
			if($t_page == 'ProductInfo' || $t_page == 'Cart' || $t_page == 'Wishlist')
			{
				...
				if($t_no_js === false)
				{
					...
					$this->v_output_buffer['GPRINT_JAVASCRIPT_CODE'] = '<script type="text/javascript" src="gm_gprint.js.php?page=' . $t_page . '&amp;mode=frontend' . $t_gm_gprint_product . $t_gm_gprint_surfaces_groups_id_parameter . $t_open_cart_dropdown . '&amp;XTCsid='.xtc_session_id().'"></script>';
				}
			}
		}		
