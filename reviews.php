...
if ($reviews_split->number_of_rows > 0) {
	...
	$smarty->assign('NAVBAR', '
			<div>
				<div class="box_left">
					'.$reviews_split->display_count(TEXT_DISPLAY_NUMBER_OF_REVIEWS).'	
				</div>	
				<div class="box_right align_right">
					'.TEXT_RESULT_PAGE.' '.$reviews_split->display_links(MAX_DISPLAY_PAGE_LINKS, xtc_get_all_get_params(array ('page', 'info', 'x', 'y'))).'
				</div>
			</div>');
...
if ($reviews_split->number_of_rows > 0) {
	...
	while ($reviews = xtc_db_fetch_array($reviews_query)) {
		// BOF GM_MOD:
		$module_data[] = array ('PRODUCTS_IMAGE' => DIR_WS_THUMBNAIL_IMAGES.$reviews['products_image'], 
...
// set cache ID
 if (!CacheCheck()) {
	...
	// BOF GM_MOD:
	$cache_id = $_SESSION['language'].$_GET['page'];
