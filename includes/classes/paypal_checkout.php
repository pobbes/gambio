<?php
/* --------------------------------------------------------------
   paypal_checkout.php 2012-02-27 misc
   Gambio GmbH
   http://www.gambio.de
...
class paypal_checkout_ORIGIN {
	...
	function paypal_checkout_ORIGIN() {
	...
	function create_account(){
		...
		$firstname = xtc_db_prepare_input(utf8_decode($_SESSION['reshash']['FIRSTNAME']));
		$lastname = xtc_db_prepare_input(utf8_decode($_SESSION['reshash']['LASTNAME']));
		$email_address = xtc_db_prepare_input($_SESSION['reshash']['EMAIL']);
		$company = xtc_db_prepare_input(utf8_decode($_SESSION['reshash']['BUSINESS']));
		$street_address = xtc_db_prepare_input(utf8_decode($_SESSION['reshash']['PAYMENTREQUEST_0_SHIPTOSTREET']));
		$postcode = xtc_db_prepare_input($_SESSION['reshash']['PAYMENTREQUEST_0_SHIPTOZIP']);
		$city = xtc_db_prepare_input(utf8_decode($_SESSION['reshash']['PAYMENTREQUEST_0_SHIPTOCITY']));
	...
	function create_shipping_address($check_customer){
		...
		$pos = strrpos($_SESSION['reshash']['PAYMENTREQUEST_0_SHIPTONAME'], ' ');
		$lenght = strlen($_SESSION['reshash']['PAYMENTREQUEST_0_SHIPTONAME']);

		$firstname = utf8_decode(substr($_SESSION['reshash']['PAYMENTREQUEST_0_SHIPTONAME'], 0, $pos));
		$lastname = utf8_decode(substr($_SESSION['reshash']['PAYMENTREQUEST_0_SHIPTONAME'], ($pos+1), $lenght));
	...
	function callback_process($data) {
		...
		if (isset ($this->data['invoice']) && is_numeric($this->data['invoice']) && ($this->data['invoice'] > 0)) {
			...
			if (xtc_db_num_rows($order_query) > 0) {
				...
				$ipn_data['first_name'] = xtc_db_input(utf8_decode($this->data['first_name']));
				$ipn_data['last_name'] = xtc_db_input(utf8_decode($this->data['last_name']));

				$ipn_data['address_name'] = xtc_db_input(utf8_decode($this->data['address_name']));
				$ipn_data['address_street'] = xtc_db_input(utf8_decode($this->data['address_street']));
				$ipn_data['address_city'] = xtc_db_input(utf8_decode($this->data['address_city']));
				$ipn_data['address_state'] = xtc_db_input(utf8_decode($this->data['address_state']));
				$ipn_data['address_zip'] = xtc_db_input($this->data['address_zip']);
				$ipn_data['address_country'] = xtc_db_input(utf8_decode($this->data['address_country']));
				$ipn_data['address_status'] = xtc_db_input($this->data['address_status']);

				$ipn_data['payer_email'] = xtc_db_input($this->data['payer_email']);
				...
				$ipn_data['business'] = xtc_db_input(utf8_decode($this->data['business']));
				$ipn_data['receiver_email'] = xtc_db_input($this->data['receiver_email']);
...
//					xtc_db_perform('paypal',$ipn_data,'update','paypal_ipn_id='.$insert_id);
	...
	function logging_status($o_id) {
		...
		$data_array = array (
 						   ...
 						   'first_name' => utf8_decode($data['FIRSTNAME']),
 						   'last_name' => utf8_decode($data['LASTNAME']),
 						   'payer_business_name' => utf8_decode($data['BUSINESS']),
 						   'address_name' => utf8_decode($data['PAYMENTREQUEST_0_SHIPTONAME']),
 						   'address_street' => utf8_decode($data['PAYMENTREQUEST_0_SHIPTOSTREET']),
 						   'address_city' => utf8_decode($data['PAYMENTREQUEST_0_SHIPTOCITY']),
 						   'address_state' => utf8_decode($data['PAYMENTREQUEST_0_SHIPTOSTATE']),
 						   'address_zip' => $data['PAYMENTREQUEST_0_SHIPTOZIP'],
 						   'address_country' => utf8_decode($data['PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE']),
...
// 2. could be an express-checkout "pending" transaction which has been Accepted in the merchant's PayPal console and needs activation in Zen Cart
  ...
  function check_api()
  {
	...
	{
		$coo_sql_log = new FileLog('payment.paypal_api_calls-'.$this->v_pp_log_token);
		$line = date("d.m.Y H:i", time()) .' | '. $call;
		$coo_sql_log->write($line . "\n");
	}
	...
	function _logTransactions($parameters)
	{
		...
		$coo_sql_log = new FileLog('payment.paypal_ipn-'.$this->v_pp_log_token);
		$coo_sql_log->write($line . "\n");
	}
