...
/* --------------------------------------------------------------
  ...
  class payment_ORIGIN {
    ...
    // class constructor
    function payment_ORIGIN($module = '') {
      ...
      if (defined('MODULE_PAYMENT_INSTALLED') && xtc_not_null(MODULE_PAYMENT_INSTALLED)) {
		...
		if($_SESSION['paypal_express_checkout']==true){
			...
			// BOF GM_MOD
			$t_gm_module_payment_installed = str_replace('paypalexpress.php', '', MODULE_PAYMENT_INSTALLED);
    ...
    if ($order->content_type == 'virtual' || ($order->content_type == 'virtual_weight') || ($order->content_type == 'mixed')) {
	...
	if($_SESSION['cart']->count_contents_virtual() == 0 && array_search('cod', $unallowed_modules) === false)
	{
