...
/* --------------------------------------------------------------
  ...
  class order_ORIGIN {
    ...
    function order_ORIGIN($order_id = '') {
    	global $xtPrice;
      $this->info = array();
      $this->totals = array();
      $this->products = array();
    ...
    function cart() {
      ...
      for ($i=0, $n=sizeof($products); $i<$n; $i++) {
        ...
        $products_price=$xtPrice->xtcGetPrice($products[$i]['id'],
                                        $format=false,
                                        $products[$i]['quantity'],
                                        $products[$i]['tax_class_id'],'')+$xtPrice->xtcFormat($_SESSION['cart']->attributes_price($products[$i]['id']),false);

		$products_price = $products_price + $_SESSION['cart']->properties_price($products[$i]['id']); #GM_MOD
        // BOF GM_MOD:
