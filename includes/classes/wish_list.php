...
class wishList_ORIGIN {
	...
	function add_cart($products_id, $qty = '1', $attributes = '', $notify = true, $p_products_properties_combis_id = 0) {
		...
		if ($this->in_cart($products_id)) {
			...
			if (isset ($_SESSION['customer_id']))	xtc_db_query("insert into ".TABLE_CUSTOMERS_WISHLIST." (customers_id, products_id, customers_basket_quantity, customers_basket_date_added) values ('".$_SESSION['customer_id']."', '".xtc_db_input($products_id)."', '".xtc_db_input($qty)."', '".date('Ymd')."')");
			...
			if (is_array($attributes)) {
				...
				while (list($option, $value) = each($attributes)) {
					...
					if (isset($_SESSION['customer_id']))
						xtc_db_query("insert into ".TABLE_CUSTOMERS_WISHLIST_ATTRIBUTES." (customers_id, products_id, products_options_id, products_options_value_id) values ('".$_SESSION['customer_id']."', '".xtc_db_input($products_id)."', '".xtc_db_input($option)."', '".xtc_db_input($value)."')");
				}
			}
		}
		$this->cleanup();
	...
	function update_quantity($products_id, $quantity = '', $attributes = '') {
		...
		// update database
		if (isset($_SESSION['customer_id'])) xtc_db_query("update " . TABLE_CUSTOMERS_WISHLIST . " set customers_basket_quantity = '" . $quantity . "' where customers_id = '" . $_SESSION['customer_id'] . "' and products_id = '" . $products_id . "'");
		...
		if (is_array($attributes)) {
			...
			while (list($option, $value) = each($attributes)) {
				$this->contents[$products_id]['attributes'][$option] = $value;
				// update database
				if (isset($_SESSION['customer_id'])) xtc_db_query("update " . TABLE_CUSTOMERS_WISHLIST_ATTRIBUTES . " set products_options_value_id = '" . $value . "' where customers_id = '" . $_SESSION['customer_id'] . "' and products_id = '" . $products_id . "' and products_options_id = '" . $option . "'");
			}
		}
	}
