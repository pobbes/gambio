...
class vrepay_kreditkarte_ORIGIN {

	// class constructor
	function vrepay_kreditkarte_ORIGIN() {

		// module description
		$this->code        = 'vrepay_kreditkarte';
		$this->title       = MODULE_PAYMENT_VREPAY_KREDITKARTE_TEXT_TITLE;
		$this->modul_title = MODULE_PAYMENT_VREPAY_KREDITKARTE_TEXT_MODUL_TITLE;
		$this->description = MODULE_PAYMENT_VREPAY_KREDITKARTE_TEXT_DESCRIPTION;
		// module enabled / disabled
		$this->enabled     = ((MODULE_PAYMENT_VREPAY_KREDITKARTE_STATUS == 'True') ? true : false);
		// sort order
		$this->sort_order  = MODULE_PAYMENT_VREPAY_KREDITKARTE_SORT_ORDER;
		// logging
		$this->log         = MODULE_PAYMENT_VREPAY_KREDITKARTE_LOG;
		$this->logging     = ((MODULE_PAYMENT_VREPAY_KREDITKARTE_LOGGING == 'True') ? true : false);
		// geozone e.g. (EU)
		$this->zone        = MODULE_PAYMENT_VREPAY_KREDITKARTE_ZONE;
		// checkout function for vrepay
		$this->form_action_url = xtc_href_link("checkout_vrepay_kreditkarte.php", '', 'SSL');

		// iframe layout
		$this->iframeFlag  = ((MODULE_PAYMENT_VREPAY_KREDITKARTE_IFRAMEFLAG == 'True') ? true : false);
		$this->iframe = MODULE_PAYMENT_VREPAY_KREDITKARTE_IFRAME;
	...
	function payment_action() {
		...
		$ft = urlencode(MODULE_PAYMENT_VREPAY_KREDITKARTE_TEXT_ERR01);
		...
		// notify URL
		$this->urlantwort = xtc_href_link("notify_vrepay_kreditkarte.php", '', 'SSL');
		$this->urlantwort = substr($this->urlantwort, 0, 254);

		// condition´s
		$this->urlagb = xtc_href_link(MODULE_PAYMENT_VREPAY_KREDITKARTE_URLAGB, '', 'SSL');
		$this->urlagb = substr($this->urlagb, 0, 254);

		// merchantno.
		$this->haendlernr = MODULE_PAYMENT_VREPAY_KREDITKARTE_INSTITUT;

		// payment type
		$this->zahlart = MODULE_PAYMENT_VREPAY_KREDITKARTE_ZAHLART;
		...
		$this->verwendung2 = substr(MODULE_PAYMENT_VREPAY_KREDITKARTE_VERWENDUNG2, 0, 25);
		...
		$md5 = md5($this->betrag.$this->referenznr.$this->zahlart.MODULE_PAYMENT_VREPAY_KREDITKARTE_ANTWGEHEIMNIS);

		// collect http-postdata
		$post = array(
			'HAENDLERNR'	=> $this->haendlernr,
			'REFERENZNR'	=> substr(MODULE_PAYMENT_VREPAY_KREDITKARTE_ORDERPREFIX, 0, 5) . $this->referenznr,
			'BETRAG'		=> $this->betrag,
		...
		$this->epay_log( "function payment_action() postdata -> " . MODULE_PAYMENT_VREPAY_KREDITKARTE_SERVER . print_r($post, TRUE) );

		// compose HTTP-Query
		$query = http_build_query($post,'','&');

		// HTTP-Basic authorisation
		$auth = MODULE_PAYMENT_VREPAY_KREDITKARTE_INSTITUT . ":" . MODULE_PAYMENT_VREPAY_KREDITKARTE_PASSWORT;

		// cURL init & options
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL,'https://'. MODULE_PAYMENT_VREPAY_KREDITKARTE_SERVER);
		...
		if ($ret == false) {
			...
			$fc = urlencode("CURL");
			$ft = urlencode(MODULE_PAYMENT_VREPAY_KREDITKARTE_TEXT_ERR03 ." [". curl_error($ch) ."]");
			$urlmsg = 'payment_error=' . $this->code . '&FEHLERCODE='. $fc . '&FEHLERTEXT=' . $ft;
...
//						'message' 	=> MODULE_PAYMENT_VREPAY_KREDITKARTE_TEXT_MSG01,
				);
		    ...
		    default:	// others = systemerror
				...
				// compose error message
				$fc = urlencode("HTTP");
				$ft = urlencode(MODULE_PAYMENT_VREPAY_KREDITKARTE_TEXT_ERR02 . " http-Code [". $info['http_code'] ."]");
				$urlmsg = 'payment_error=' . $this->code . '&FEHLERCODE='. $fc . '&FEHLERTEXT=' . $ft;
	...
	function check() {
		...
		// query for checking module stat --> MODULE_PAYMENT_VREPAY_KREDITKARTE_STATUS
		if (!isset($this->_check)) {
			$check_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_VREPAY_KREDITKARTE_STATUS'");
	...
	function install() {
		...
		$pos2 = strpos(MODULE_PAYMENT_INSTALLED, "vrepay_giropay");
		...
		// xtc Standard
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_VREPAY_KREDITKARTE_STATUS', 'True', '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_VREPAY_KREDITKARTE_ZONE', '0', '6', '14', 'xtc_get_zone_class_title', 'xtc_cfg_pull_down_zone_classes(', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VREPAY_KREDITKARTE_ALLOWED', '', '6', '0', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VREPAY_KREDITKARTE_SORT_ORDER', '10', '6', '16', now())");
		// vrepay_dialog
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VREPAY_KREDITKARTE_SERVER', 'payinte.vr-epay.de/pbr/transaktion', '6', '2', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VREPAY_KREDITKARTE_INSTITUT', 'vrepay_merchant', '6', '3', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VREPAY_KREDITKARTE_PASSWORT', 'vrepay_password', '6', '4', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VREPAY_KREDITKARTE_ZAHLART', 'KAUFEN', '6', '5', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VREPAY_KREDITKARTE_URLAGB', 'popup_content.php?coID=3', '6', '7', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VREPAY_KREDITKARTE_ORDERPREFIX', '', '6', '8', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VREPAY_KREDITKARTE_ANTWGEHEIMNIS', 'your_secret', '6', '8', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VREPAY_KREDITKARTE_VERWENDUNG2', 'enjoy shopping', '6', '10', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_VREPAY_KREDITKARTE_IFRAMEFLAG', 'False', '6', '13', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VREPAY_KREDITKARTE_IFRAME', 'width=\"568px\" height=\"540px\" frameborder=\"0\"', '6', '11', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VREPAY_KREDITKARTE_LOG', '/tmp/vrepay_kreditkarte.log', '6', '12', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_VREPAY_KREDITKARTE_LOGGING', 'False', '6', '13', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
	}
	...
	function keys() {
		return array(
			'MODULE_PAYMENT_VREPAY_KREDITKARTE_STATUS',
			'MODULE_PAYMENT_VREPAY_KREDITKARTE_ZONE',
			'MODULE_PAYMENT_VREPAY_KREDITKARTE_ALLOWED',
			'MODULE_PAYMENT_VREPAY_KREDITKARTE_SORT_ORDER',
			'MODULE_PAYMENT_VREPAY_KREDITKARTE_SERVER',
			'MODULE_PAYMENT_VREPAY_KREDITKARTE_INSTITUT',
			'MODULE_PAYMENT_VREPAY_KREDITKARTE_PASSWORT',
			'MODULE_PAYMENT_VREPAY_KREDITKARTE_ZAHLART',
			'MODULE_PAYMENT_VREPAY_KREDITKARTE_URLAGB',
			'MODULE_PAYMENT_VREPAY_KREDITKARTE_ORDERPREFIX',
			'MODULE_PAYMENT_VREPAY_KREDITKARTE_ANTWGEHEIMNIS',
			'MODULE_PAYMENT_VREPAY_KREDITKARTE_VERWENDUNG2',
			'MODULE_PAYMENT_VREPAY_KREDITKARTE_IFRAMEFLAG',
			'MODULE_PAYMENT_VREPAY_KREDITKARTE_IFRAME',
			'MODULE_PAYMENT_VREPAY_KREDITKARTE_LOG',
			'MODULE_PAYMENT_VREPAY_KREDITKARTE_LOGGING'
		);
	}
...
MainFactory::load_origin_class('vrepay_kreditkarte');
?>