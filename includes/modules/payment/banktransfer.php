<?php
/* --------------------------------------------------------------
   banktransfer.php 2010-06-17 gm
   Gambio GmbH
   http://www.gambio.de
   ...
   (c) 2003 XT-Commerce - community made shopping http://www.xt-commerce.com ($Id: banktransfer.php 1122 2005-07-26 10:16:27Z mz $)
  ...
  class banktransfer_ORIGIN {

    var $code, $title, $description, $enabled, $gm_check_blz;


function banktransfer_ORIGIN() {
	global $order;
	...
	//bof gm
	$this->gm_check_blz = MODULE_PAYMENT_BANKTRANSFER_DATABASE_BLZ;
	//eof gm
}
    ...
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_BANKTRANSFER_ZONE > 0) ) {
        $check_flag = false;
        $check_query = xtc_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_BANKTRANSFER_ZONE . "' and zone_country_id = '" . (int)$order->billing['country']['id'] . "' order by zone_id");
                         ...
                         'description'=>$this->info,
      	                 'fields' => array(array('title' => MODULE_PAYMENT_BANKTRANSFER_TEXT_NOTE,
      	                                   ...
      	                                   array('title' => MODULE_PAYMENT_BANKTRANSFER_TEXT_BANK_BLZ,
      	                                         'field' => xtc_draw_input_field('banktransfer_blz', htmlentities_wrapper($_GET['banktransfer_blz']), 'size="8" maxlength="8"')),
      	                                   array('title' => MODULE_PAYMENT_BANKTRANSFER_TEXT_BANK_NUMBER,
      	                                         'field' => xtc_draw_input_field('banktransfer_number',  htmlentities_wrapper($_GET['banktransfer_number']), 'size="16" maxlength="32"')),
      	                                   array('title' => MODULE_PAYMENT_BANKTRANSFER_TEXT_BANK_NAME,
      	                                         'field' => xtc_draw_input_field('banktransfer_bankname',  $_GET['banktransfer_bankname'], 'size="16" maxlength="32"')),
      	                                   array('title' => '',
      	                                         'field' => xtc_draw_hidden_field('recheckok', htmlentities_wrapper($_POST['recheckok'])))
      	                                   ));
    ...
    function pre_confirmation_check(){
      ...
      if ($_POST['banktransfer_fax'] == false  && $_POST['recheckok'] != 'true') {
		...
		// bof gm
		// if false, all entered bank data OK
		if(MODULE_PAYMENT_BANKTRANSFER_DATACHECK == 'false') {
			$banktransfer_result = 0;
			if(isset($_POST['banktransfer_blz']) && empty($_POST['banktransfer_blz']))
			{
				$banktransfer_result = 8;
			} else if(isset($_POST['banktransfer_number']) && empty($_POST['banktransfer_number']))
			{
				$banktransfer_result = 9;
			}
		}
		//eof gm
    ...
    function after_process() {
      	...
      	xtc_db_query("
	      	...
	      	VALUES (
	      		'" . $insert_id . "', 
	      		'" . xtc_db_input($_POST['banktransfer_blz']) 		."', 
	      		'" . xtc_db_input($_POST['banktransfer_bankname']) 	."', 
	      		'" . xtc_db_input($_POST['banktransfer_number']) 	."', 
	      		'" . xtc_db_input($_POST['banktransfer_owner']) 	."', 
	      		'" . xtc_db_input($_POST['banktransfer_status']) 	."', 
	      		'" . xtc_db_input($_POST['banktransfer_prz']) 		."'
      		)"
     	);
      
      if ($_POST['banktransfer_fax'])
        xtc_db_query("update banktransfer set banktransfer_fax = '" . xtc_db_input($_POST['banktransfer_fax']) ."' where orders_id = '" . $insert_id . "'");

        if ($this->order_status) xtc_db_query("UPDATE ". TABLE_ORDERS ." SET orders_status='".(int)$this->order_status."' WHERE orders_id='".$insert_id."'");
    }
    ...
    function install() {
      xtc_db_query("insert into " . TABLE_CONFIGURATION . " ( configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_BANKTRANSFER_STATUS', 'True', '6', '1', 'gm_cfg_select_option(array(\'True\', \'False\'), ', now())");
      ...
      xtc_db_query("insert into " . TABLE_CONFIGURATION . " ( configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_BANKTRANSFER_FAX_CONFIRMATION', 'false',  '6', '2', 'gm_cfg_select_option(array(\'true\', \'false\'), ', now())");
      xtc_db_query("insert into " . TABLE_CONFIGURATION . " ( configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_BANKTRANSFER_DATABASE_BLZ', 'false', '6', '0', 'gm_cfg_select_option(array(\'true\', \'false\'), ', now())");
	  ...
	  xtc_db_query("insert into " . TABLE_CONFIGURATION . " ( configuration_key, configuration_value, configuration_group_id, sort_order, last_modified, date_added, use_function, set_function) VALUES('MODULE_PAYMENT_BANKTRANSFER_DATACHECK', 'false', 6, 3, NULL, '2011-05-19 08:19:02', NULL, 'gm_cfg_select_option(array(''true'', ''false''), ')");
    }
