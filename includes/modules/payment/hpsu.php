...
class hpsu_ORIGIN {
  ...
  function update_status() /*{{{*/
  {
		...
		if (($this->enabled == true) && ((int) MODULE_PAYMENT_HPSU_ZONE > 0)) {
			$check_flag = false;
			$check_query = xtc_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_HPSU_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
  ...
  function selection() /*{{{*/
  {
    ...
    if (MODULE_PAYMENT_HPSU_TRANSACTION_MODE == 'LIVE' || strpos(MODULE_PAYMENT_HPSU_TEST_ACCOUNT, $order->customer['email_address']) !== false) {
      $content = array (
        array (
          'title' => '',
          'field' => '',
  ...
  function check() /*{{{*/
  {
		if (!isset ($this->_check)) {
			$check_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_HPSU_STATUS'");
  ...
  {
		return array (
			'MODULE_PAYMENT_HPSU_STATUS',
      'MODULE_PAYMENT_HPSU_SECURITY_SENDER',
      'MODULE_PAYMENT_HPSU_USER_LOGIN',
      'MODULE_PAYMENT_HPSU_USER_PWD',
      'MODULE_PAYMENT_HPSU_TRANSACTION_CHANNEL',
      'MODULE_PAYMENT_HPSU_TRANSACTION_MODE',
      'MODULE_PAYMENT_HPSU_DIRECT_MODE',
      'MODULE_PAYMENT_HPSU_TEST_ACCOUNT',
			'MODULE_PAYMENT_HPSU_PROCESSED_STATUS_ID',
			'MODULE_PAYMENT_HPSU_PENDING_STATUS_ID',
			'MODULE_PAYMENT_HPSU_CANCELED_STATUS_ID',
			'MODULE_PAYMENT_HPSU_NEWORDER_STATUS_ID',
			'MODULE_PAYMENT_HPSU_SORT_ORDER',
			'MODULE_PAYMENT_HPSU_ALLOWED',
      'MODULE_PAYMENT_HPSU_ZONE',
      #'MODULE_PAYMENT_HPSU_',
		);
	}/*}}}*/

}

MainFactory::load_origin_class('hpsu');
?>