	...
	function selection() {
		...
		if (MODULE_PAYMENT_BILLSAFE_2_LOG == 'True') {
			if (MODULE_PAYMENT_BILLSAFE_2_LOG_TYPE == 'Echo') {
				...
				$bs->setLogger(new Billsafe_LoggerFile(DIR_FS_CATALOG.'logfiles/BillSAFE_'.date('YmdHis').'.log'));
			}
		}
		...
		if(sizeof($order->delivery) != sizeof($order->billing)) {
			...
			if (is_array($order->billing)) {
				foreach ($order->billing as $key => $val) {
					if ($order->billing[$key] != $order->delivery[$key]) xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error='.$this->code.'&error_message='.stripslashes(html_entity_decode(MODULE_PAYMENT_BILLSAFE_2_ERROR_MESSAGE_ADDRESS)), 'SSL'));
				}
			}
		}
//		if ($order->billing['company'] != '') xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error='.$this->code.'&error_message='.stripslashes(html_entity_decode(MODULE_PAYMENT_BILLSAFE_2_ERROR_MESSAGE_COMPANY)), 'SSL'));
		...
		if (empty($_GET['token'])) {
			...
			for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
				if (is_array($order->products[$i]['attributes'])) {
					for ($ia = 0, $na = sizeof($order->products[$i]['attributes']); $ia < $na; $ia++) {
						for ($iao = 0, $nao = sizeof($order->products[$i]['attributes'][$ia]['option']); $iao < $nao; $iao++) {
							$att .= ' - '.$order->products[$i]['attributes'][$ia]['option'].': '.$order->products[$i]['attributes'][$ia]['value'];
						}
					}
				}
				...
				if($t_combis_id != '')
				{
					$t_properties_data_array = $coo_properties_view->v_coo_properties_control->get_properties_combis_details($t_combis_id, $_SESSION['languages_id']);
					for($k = 0; $k < count($t_properties_data_array); $k++)
					{
						$att .= ' - '.$t_properties_data_array[$k]['properties_name'].': '.$t_properties_data_array[$k]['values_name'];
					}
				}
								
				$article[$i]['number'] = $order->products[$i]['model'].$att;
				if(empty($article[$i]['number']))
				{
					$article[$i]['number'] = $order->products[$i]['id'];
				}
			...
			if ($shipping_cost > 0) {
				...
				$shipping_cost = $order->info['shipping_cost'] * $xtPrice->currencies[$currency]['value'];
				$shipping_cost = xtc_add_tax($shipping_cost, $shipping_tax_rate);
				$article[$i]['grossPrice'] = number_format($shipping_cost, 2, '.', '');
				$article[$i]['tax'] = number_format($shipping_tax_rate, 2, '.', '');
				$shipping_cost = round($shipping_cost, $xtPrice->get_decimal_places($currency));
			}
			...
			if (xtc_db_num_rows($customer_query)) $customer = xtc_db_fetch_array($customer_query);
			if ($_SESSION['customers_status']['customers_status_show_price_tax'] == 0 && $_SESSION['customers_status']['customers_status_add_tax_ot'] == 1) {
				$total = $order->info['subtotal'] + $order->info['tax'] + $_SESSION['discount_value'] - $_SESSION['voucher_value'] - $_SESSION['coupon_value'] + $_SESSION['schg_value'];
			} else {
				$total = $order->info['subtotal'] + $_SESSION['discount_value'] - $_SESSION['voucher_value'] - $_SESSION['coupon_value'] + $_SESSION['schg_value'];
			}
			$total = round($xtPrice->xtcCalculateCurrEx($total, $_SESSION['currency']), $xtPrice->get_decimal_places($_SESSION['currency']));
			$info_tax = $order->info['tax']; 
			$shipping_tax = round(($order->info['shipping_cost'] / 100) * $shipping_tax_rate, 2);
			$total = number_format(round(($total + $shipping_cost), $xtPrice->get_decimal_places($currency)), 2, '.', '');
			$info_tax = number_format(round(($info_tax + $shipping_tax + $schg_tax), $xtPrice->get_decimal_places($currency)), 2, '.', '');;
...
//			$_SESSION['bs_article'] = $article;
			...
			if ($response->ack == 'OK') {
				...
				xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error='.$this->code.'&error_message='.stripslashes(html_entity_decode($message)), 'SSL'));
			}
		} else {
			$token = $_GET['token'];
			$check_query = xtc_db_query('SELECT token FROM billsafe_orders_2 WHERE token = "'.xtc_db_input($token).'"');
			$check_token = xtc_db_num_rows($check_query);
			if ($check_token == 1) xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error='.$this->code.'&error_message='.stripslashes(html_entity_decode(MODULE_PAYMENT_BILLSAFE_2_ERROR_MESSAGE_COMMON)), 'SSL'));
			require_once (DIR_FS_CATALOG.'includes/classes/billsafe_2/billsafe_2.php');
			$bs = new Billsafe_Sdk(DIR_FS_CATALOG.'includes/classes/billsafe_2/ini.php');
			if($_SESSION['language_charset'] == 'iso-8859-1' || $_SESSION['language_charset'] == 'iso-8859-15') {
				$bs->setUtf8Mode(false);
			...
			if ($this->response->ack == 'OK' && $this->response->status == 'ACCEPTED') {
				...
				xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error='.$this->code.'&error_message='.stripslashes(html_entity_decode(MODULE_PAYMENT_BILLSAFE_2_ERROR_MESSAGE_COMMON)), 'SSL'));
			}
		}
		return true;
	}
	...
	function after_process() {
		...
		if ($this->response->ack == 'OK' && $this->response->status == 'ACCEPTED') {
			...
			xtc_db_query('INSERT INTO billsafe_orders_2 (id, orderid, transactionid, token, billsafeStatus, paymentStatus,date) VALUES (NULL,"'.xtc_db_input($insert_id).'", "'.xtc_db_input($this->response->transactionId).'", "'.xtc_db_input($token).'", "'.xtc_db_input($this->response->status).'", NULL, "'.date('Y-m-d H:i:s').'")');
			...
			xtc_db_query ('INSERT INTO orders_status_history (orders_status_history_id, orders_id, orders_status_id, date_added, customer_notified, comments) VALUES (NULL, "'.(int)$insert_id.'", "1", "'.date('Y-m-d H:i:s').'", "0", "'.xtc_db_input($comments).'")');
	...
	function install() {
		...
		{
			if(gm_get_conf('GM_LOGO_MAIL_USE') == '1')
			{
				$logo_url = HTTPS_CATALOG_SERVER . DIR_WS_CATALOG . 'images/logos/' . gm_get_conf('GM_LOGO_MAIL');
			}
		}
		...
		xtc_db_query('INSERT INTO '.TABLE_CONFIGURATION.' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2_SHOP_LOGO_URL", "'.xtc_db_input($logo_url).'", "6", "0", now())');
		xtc_db_query('INSERT INTO '.TABLE_CONFIGURATION.' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2_BILLSAFE_LOGO_URL", "'.xtc_db_input($billsafe_logo).'", "6", "0", now())');
	...
	function get_shipping_tax_rate($shipping_id) {
		$check_query = xtc_db_query('SELECT configuration_value FROM '.TABLE_CONFIGURATION.' WHERE configuration_key = "MODULE_SHIPPING_'.xtc_db_input($shipping_id).'_TAX_CLASS"');
...
MainFactory::load_origin_class('billsafe_2');
?>