...
class hpdc_ORIGIN {
	...
	function selection() /*{{{*/
	{
		...
		if (MODULE_PAYMENT_HPDC_TRANSACTION_MODE == 'LIVE' || strpos(MODULE_PAYMENT_HPDC_TEST_ACCOUNT, $order->customer['email_address']) !== false) {
			...
			if (MODULE_PAYMENT_HPDC_MODULE_MODE == 'DIRECT'){
				...
				$lastCCard 			= $this->hp->loadMEMO($_SESSION['customer_id'], 'heidelpay_last_debitcard');
				$card_expire_month 	= $this->hp->loadMEMO($_SESSION['customer_id'], 'heidelpay_last_debitcard_expire_month');
				$card_expire_year  	= $this->hp->loadMEMO($_SESSION['customer_id'], 'heidelpay_last_debitcard_expire_year');
				if(($card_expire_year > date('Y')) OR ( $card_expire_year == date('Y') AND $card_expire_month >= date('m'))){
					...
					$content[] = array (
            'title' => '<input type="checkbox" name="hpdcReuseDCard" value="1" checked>'.$gender.' '.$name.', '.MODULE_PAYMENT_HPDC_REUSE_DCARD.$lastDCard,
            'field' => '',
					);
				}
			}
			$content[] = array (
        'title' => '',
        'field' => ''.$hpIframe.'<input type="hidden" name="hpdcFirstStep" value="1">',
			);
		} else {
			$content = array (
			array (
	...
	function pre_confirmation_check() /*{{{*/
	{
		...
		if (MODULE_PAYMENT_HPDC_TRANSACTION_MODE == 'LIVE' || strpos(MODULE_PAYMENT_HPDC_TEST_ACCOUNT, $order->customer['email_address']) !== false) {
			if (defined('MODULE_PAYMENT_HPDC_MODULE_MODE')) {
				$_SESSION['hpModuleMode'] = MODULE_PAYMENT_HPDC_MODULE_MODE;
			} else {
				$_SESSION['hpModuleMode'] = 'AFTER' ;
			}
			if ($_POST['hpdcReuseDCard'] == 1){
	...
	function confirmation() /*{{{*/
	{
		...
		if (!empty($lastDCard)){
			...
			$name = $_SESSION['customer_last_name'];
			$content[] = array (
        'title' => MODULE_PAYMENT_HPDC_WILLUSE_DCARD.$lastDCard,
        'field' => '',
			);
		} else {
			return false;
	...
	{
		return array (
			'MODULE_PAYMENT_HPDC_STATUS',
      'MODULE_PAYMENT_HPDC_SECURITY_SENDER',
      'MODULE_PAYMENT_HPDC_USER_LOGIN',
      'MODULE_PAYMENT_HPDC_USER_PWD',
      'MODULE_PAYMENT_HPDC_TRANSACTION_CHANNEL',
      'MODULE_PAYMENT_HPDC_TRANSACTION_MODE',
      'MODULE_PAYMENT_HPDC_DIRECT_MODE',
			'MODULE_PAYMENT_HPDC_PAY_MODE',
      'MODULE_PAYMENT_HPDC_TEST_ACCOUNT',
			'MODULE_PAYMENT_HPDC_PROCESSED_STATUS_ID',
			'MODULE_PAYMENT_HPDC_PENDING_STATUS_ID',
			'MODULE_PAYMENT_HPDC_CANCELED_STATUS_ID',
			'MODULE_PAYMENT_HPDC_NEWORDER_STATUS_ID',
			'MODULE_PAYMENT_HPDC_SORT_ORDER',
			'MODULE_PAYMENT_HPDC_ALLOWED',
      'MODULE_PAYMENT_HPDC_ZONE',
		#'MODULE_PAYMENT_HPDC_',
		);
	}/*}}}*/
