...
class worldpay_ORIGIN {
	var $code, $title, $description, $enabled;

	// class constructor
	function worldpay_ORIGIN() {
		global $order;
		$this->code = 'worldpay';
	...
	function process_button() {
		...
		$total = number_format((double)$xtPrice->xtcCalculateCurr($order->info['total']), (double)$xtPrice->get_decimal_places($_SESSION['currency']), '.', '');
		...
		if (MODULE_PAYMENT_WORLDPAY_USEMD5 == '1') {
			...
			$md5_signature = MODULE_PAYMENT_WORLDPAY_MD5KEY.':'. (number_format((double)$order->info['total'] * $currencies->get_value($currency), (double)$currencies->get_decimal_places($currency), '.', '')).':'.$language_code.':'.$order->customer['email_address'];
	...
	function install() {
		xtc_db_query("insert into ".TABLE_CONFIGURATION." (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_WORLDPAY_STATUS', 'True', '6', '1', 'gm_cfg_select_option(array(\'True\', \'False\'), ', now())");
		...
		xtc_db_query("insert into ".TABLE_CONFIGURATION." (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_WORLDPAY_USEPREAUTH', 'False', '6', '3', 'gm_cfg_select_option(array(\'True\', \'False\'), ', now())");
...
MainFactory::load_origin_class('worldpay');
?>