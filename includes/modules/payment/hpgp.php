...
class hpgp_ORIGIN {
	...
	function selection() /*{{{*/
	{
		...
		if (MODULE_PAYMENT_HPGP_TRANSACTION_MODE == 'LIVE' || strpos(MODULE_PAYMENT_HPGP_TEST_ACCOUNT, $order->customer['email_address']) !== false) {
			if (MODULE_PAYMENT_HPGP_MODULE_MODE == 'NOWPF'){
				$content = array (
				array (
            'title' => 'Konto-Nr.',
            'field' => '<input autocomplete="off" value="" style="width: 200px;" maxlength="50" name="hpgp[AccountNumber]" type="TEXT">',
				),
				array (
            'title' => 'Bankleitzahl',
            'field' => '<input value="" style="width: 200px;" maxlength="50" name="hpgp[BankCode]" type="TEXT">',
				),
				array (
            'title' => 'Kontoinhaber',
            'field' => '<input value="" style="width: 200px;" maxlength="50" name="hpgp[Holder]" type="TEXT">',
				),
				);
			} else {
				$content = array (
	...
	function pre_confirmation_check() /*{{{*/
	{
		GLOBAL $order;
		if (MODULE_PAYMENT_HPGP_TRANSACTION_MODE == 'LIVE' || strpos(MODULE_PAYMENT_HPGP_TEST_ACCOUNT, $order->customer['email_address']) !== false) {
			if (defined('MODULE_PAYMENT_HPGP_MODULE_MODE')) {
				$_SESSION['hpModuleMode'] = MODULE_PAYMENT_HPGP_MODULE_MODE;
			} else {
	...
	function install() /*{{{*/
	{
      ...
      'configuration_value'     => '31HA07BC81A71E2A47DA662C5EDD1112',
		);
		$inst[] = array(
	...
	{
		return array (
			'MODULE_PAYMENT_HPGP_STATUS',
      'MODULE_PAYMENT_HPGP_SECURITY_SENDER',
      'MODULE_PAYMENT_HPGP_USER_LOGIN',
      'MODULE_PAYMENT_HPGP_USER_PWD',
      'MODULE_PAYMENT_HPGP_TRANSACTION_CHANNEL',
      'MODULE_PAYMENT_HPGP_TRANSACTION_MODE',
      'MODULE_PAYMENT_HPGP_TEST_ACCOUNT',
			'MODULE_PAYMENT_HPGP_PROCESSED_STATUS_ID',
			'MODULE_PAYMENT_HPGP_PENDING_STATUS_ID',
			'MODULE_PAYMENT_HPGP_CANCELED_STATUS_ID',
			'MODULE_PAYMENT_HPGP_NEWORDER_STATUS_ID',
			'MODULE_PAYMENT_HPGP_SORT_ORDER',
			'MODULE_PAYMENT_HPGP_ALLOWED',
      'MODULE_PAYMENT_HPGP_ZONE',
		#'MODULE_PAYMENT_HPGP_',
		);
	}/*}}}*/
