...
class hppp_ORIGIN {
  ...
  function hppp_ORIGIN() /*{{{*/
  {
    ...
    $this->payCode = 'pp';
		...
		$this->version = '1.0';
		$this->title = MODULE_PAYMENT_HPPP_TEXT_TITLE;
		$this->description = MODULE_PAYMENT_HPPP_TEXT_DESC;
		$this->sort_order = MODULE_PAYMENT_HPPP_SORT_ORDER;
		$this->enabled = ((MODULE_PAYMENT_HPPP_STATUS == 'True') ? true : false);
    $this->info = MODULE_PAYMENT_HPPP_TEXT_INFO;
    #$this->form_action_url = 'checkout_success.php';
		$this->tmpOrders = false;
    $this->tmpStatus = MODULE_PAYMENT_HPPP_NEWORDER_STATUS_ID;
    $this->order_status = MODULE_PAYMENT_HPPP_NEWORDER_STATUS_ID;
  ...
  function selection() /*{{{*/
  {
    ...
    if (MODULE_PAYMENT_HPPP_TRANSACTION_MODE == 'LIVE' || strpos(MODULE_PAYMENT_HPPP_TEST_ACCOUNT, $order->customer['email_address']) !== false) {
      ...
      $content = array (
        array (
          'title' => '',
          'field' => MODULE_PAYMENT_HPPP_DEBUGTEXT,
        ),
      );
    }
  ...
  function get_error() /*{{{*/
  {
		...
		$error = array (
			'title' => MODULE_PAYMENT_HPPP_TEXT_ERROR,
  ...
  {
		return array (
			'MODULE_PAYMENT_HPPP_STATUS',
      'MODULE_PAYMENT_HPPP_SECURITY_SENDER',
      'MODULE_PAYMENT_HPPP_USER_LOGIN',
      'MODULE_PAYMENT_HPPP_USER_PWD',
      'MODULE_PAYMENT_HPPP_TRANSACTION_CHANNEL',
      'MODULE_PAYMENT_HPPP_TRANSACTION_MODE',
      'MODULE_PAYMENT_HPPP_TEST_ACCOUNT',
			'MODULE_PAYMENT_HPPP_PROCESSED_STATUS_ID',
			'MODULE_PAYMENT_HPPP_PENDING_STATUS_ID',
			'MODULE_PAYMENT_HPPP_CANCELED_STATUS_ID',
			'MODULE_PAYMENT_HPPP_NEWORDER_STATUS_ID',
			'MODULE_PAYMENT_HPPP_SORT_ORDER',
			'MODULE_PAYMENT_HPPP_ALLOWED',
      'MODULE_PAYMENT_HPPP_ZONE',
      #'MODULE_PAYMENT_HPPP_',
		);
	}/*}}}*/

}

MainFactory::load_origin_class('hppp');
?>