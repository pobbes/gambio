...
/* --------------------------------------------------------------
  ...
  class ot_total_ORIGIN {
    var $title, $output;

    function ot_total_ORIGIN() {
      $this->code = 'ot_total';
    ...
    function install() {
      xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,configuration_group_id, sort_order, set_function, date_added) values ('MODULE_ORDER_TOTAL_TOTAL_STATUS', 'true', '6', '1','gm_cfg_select_option(array(\'true\', \'false\'), ', now())");
...
MainFactory::load_origin_class('ot_total');
?>