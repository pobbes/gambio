 ...
 */
  ...
  class gambioultra_ORIGIN {
    var $code, $title, $description, $enabled, $num_zones;

/**
 * class constructor
 */
    function gambioultra_ORIGIN() {
      $this->code = 'gambioultra';
      $this->title = MODULE_SHIPPING_GAMBIOULTRA_TEXT_TITLE;
      $this->description = MODULE_SHIPPING_GAMBIOULTRA_TEXT_DESCRIPTION;
      $this->sort_order = MODULE_SHIPPING_GAMBIOULTRA_SORT_ORDER;
      $this->icon = '';
      $this->tax_class = MODULE_SHIPPING_GAMBIOULTRA_TAX_CLASS;
      $this->enabled = ((MODULE_SHIPPING_GAMBIOULTRA_STATUS == 'True') ? true : false);
 ...
 */
    ...
    function quote($method = '') {
      ...
      for ($i=1; $i<=$this->num_zones; $i++) {
        $countries_table = constant('MODULE_SHIPPING_GAMBIOULTRA_COUNTRIES_' . $i);
      ...
      if ($dest_zone == 0) {
        ...
        $shipping = -1;
        $zones_cost = constant('MODULE_SHIPPING_GAMBIOULTRA_COST_' . $dest_zone);
        ...
        for ($i=0; $i<$size; $i+=2) {
          if ($shipping_weight <= $zones_table[$i]) {
            ...
            $shipping_method = MODULE_SHIPPING_GAMBIOULTRA_TEXT_WAY . ' ' . $dest_country .' ';
        ...
        if ($shipping == -1) {
          $error = true;
          $shipping_cost 		= 0;
          $shipping_method 	= MODULE_SHIPPING_GAMBIOULTRA_UNDEFINED_RATE;
        } 
        else {
        	$zones_cost_full 	= $shipping + constant('MODULE_SHIPPING_GAMBIOULTRA_HANDLING_' . $dest_zone);
          $shipping_cost 		= $zones_cost_full + $nc_ultra_data['costs'];
          if($zones_cost_full > 0) {
          	$shipping_method .= $xtPrice->xtcFormat($zones_cost_full, true, $this->tax_class, true) . $nc_ultra_details;
          }
          else {
          	$shipping_method .= $nc_ultra_details;
          }
      ...
      $this->quotes = array('id' => $this->code,
                            'module' => MODULE_SHIPPING_GAMBIOULTRA_TEXT_TITLE,
      ...
      if ($error == true) $this->quotes['error'] = MODULE_SHIPPING_GAMBIOULTRA_UNDEFINED_RATE;
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_GAMBIOULTRA_STATUS'");
    ...
    function install() {
      xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ('MODULE_SHIPPING_GAMBIOULTRA_STATUS', 'True', '6', '0', 'gm_cfg_select_option(array(\'True\', \'False\'), ', now())");
      xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_GAMBIOULTRA_ALLOWED', '', '6', '0', now())");
      xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_SHIPPING_GAMBIOULTRA_TAX_CLASS', '0', '6', '0', 'xtc_get_tax_class_title', 'xtc_cfg_pull_down_tax_classes(', now())");
      xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_GAMBIOULTRA_SORT_ORDER', '0', '6', '0', now())");
      for ($i = 1; $i <= $this->num_zones; $i++) {
        $default_countries = '';
        if ($i == 1) {
          $default_countries = 'DE,CH,AT';
        }
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_GAMBIOULTRA_COUNTRIES_" . $i ."', '" . $default_countries . "', '6', '0', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_GAMBIOULTRA_COST_" . $i ."', '', '6', '0', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_GAMBIOULTRA_HANDLING_" . $i."', '0', '6', '0', now())");
      }
    }
    ...
    function keys() {
      $keys = array('MODULE_SHIPPING_GAMBIOULTRA_STATUS','MODULE_SHIPPING_GAMBIOULTRA_ALLOWED', 'MODULE_SHIPPING_GAMBIOULTRA_TAX_CLASS', 'MODULE_SHIPPING_GAMBIOULTRA_SORT_ORDER');

      for ($i=1; $i<=$this->num_zones; $i++) {
        $keys[] = 'MODULE_SHIPPING_GAMBIOULTRA_COUNTRIES_' . $i;
        $keys[] = 'MODULE_SHIPPING_GAMBIOULTRA_COST_' . $i;
        $keys[] = 'MODULE_SHIPPING_GAMBIOULTRA_HANDLING_' . $i;
      }
...
MainFactory::load_origin_class('gambioultra');
?>