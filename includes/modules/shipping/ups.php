...
function quote($method = '') {
	...
	if ($dest_zone == 0) {
		$error = true;
		} elseif (($dest_zone == 1) && ((round($_SESSION['cart']->show_total(),2)) >= MODULE_SHIPPING_UPS_FREEAMOUNT)) {
			$freeship = true;
			$shipping = 0;
	...
	if ($error == true) $this->quotes['error'] = MODULE_SHIPPING_UPS_UNDEFINED_RAT;
...
function install() {
xtc_db_query("insert into " . TABLE_CONFIGURATION . " ( configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) VALUES ('MODULE_SHIPPING_UPS_STATUS', 'True', '6', '0', 'gm_cfg_select_option(array(\'True\', \'False\'), ', now())");
...
MainFactory::load_origin_class('ups');
?>