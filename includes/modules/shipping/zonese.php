 ...
 * class constructor
 */
    function zonese_ORIGIN() {
      $this->code = 'zonese';
      $this->title = MODULE_SHIPPING_ZONESE_TEXT_TITLE;
      $this->description = MODULE_SHIPPING_ZONESE_TEXT_DESCRIPTION;
      $this->sort_order = MODULE_SHIPPING_ZONESE_SORT_ORDER;
      $this->icon = '';
      $this->tax_class = MODULE_SHIPPING_ZONESE_TAX_CLASS;
      $this->enabled = ((MODULE_SHIPPING_ZONESE_STATUS == 'True') ? true : false);

/**
 * CUSTOMIZE THIS SETTING FOR THE NUMBER OF ZONESE NEEDED
 *
 * + CUSTOMIZE THE SETTING IN lang/LANGUAGE/modules/shipping/zonese.php
 */
      $this->num_zones = 10;
    }
 ...
 */
    function quote($method = '') {
      ...
      if ($dest_zone == 0) {
        ...
        if ($shipping == -1) {
            $error = true;
          $shipping_cost = 0;
          $shipping_method = MODULE_SHIPPING_ZONESE_UNDEFINED_RATE;
        } else {
          $shipping_cost = (($shipping * $shipping_num_boxes)+  (constant('MODULE_SHIPPING_ZONESE_HANDLING_' . $dest_zone)* $shipping_num_boxes));
        }
      }

      $this->quotes = array('id' => $this->code,
                            'module' => MODULE_SHIPPING_ZONESE_TEXT_TITLE,
      ...
      if ($error == true) $this->quotes['error'] = MODULE_SHIPPING_ZONESE_UNDEFINED_RATE;
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_ZONESE_STATUS'");
    ...
    function install() {
      ...
      xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ('MODULE_SHIPPING_ZONESE_STATUS', 'True', '6', '0', 'gm_cfg_select_option(array(\'True\', \'False\'), ', now())");
      xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_ZONESE_ALLOWED', '', '6', '0', now())");
      xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_SHIPPING_ZONESE_TAX_CLASS', '0', '6', '0', 'xtc_get_tax_class_title', 'xtc_cfg_pull_down_tax_classes(', now())");
      xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_ZONESE_SORT_ORDER', '0', '6', '0', now())");
      for ($i = 1; $i <= $this->num_zones; $i++) {
        $default_countries = '';
        if ($i == 1) {
          $default_countries = 'DE,AT,CH';
        }
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_ZONESE_COUNTRIES_" . $i ."', '" . $default_countries . "', '6', '0', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_ZONESE_COST_" . $i ."', '', '6', '0', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_ZONESE_HANDLING_" . $i."', '0', '6', '0', now())");
      }
    }
    ...
    function keys() {
      $keys = array('MODULE_SHIPPING_ZONESE_STATUS','MODULE_SHIPPING_ZONESE_ALLOWED', 'MODULE_SHIPPING_ZONESE_TAX_CLASS', 'MODULE_SHIPPING_ZONESE_SORT_ORDER');

      for ($i=1; $i<=$this->num_zones; $i++) {
        $keys[] = 'MODULE_SHIPPING_ZONESE_COUNTRIES_' . $i;
        $keys[] = 'MODULE_SHIPPING_ZONESE_COST_' . $i;
        $keys[] = 'MODULE_SHIPPING_ZONESE_HANDLING_' . $i;
      }
...
MainFactory::load_origin_class('zonese');
?>