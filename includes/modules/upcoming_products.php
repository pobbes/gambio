<?php
/* --------------------------------------------------------------
   upcoming_products.php 2008-08-10 gambio
   Gambio OHG
   http://www.gambio.de
	...
	if (xtc_db_num_rows($expected_query,true) > 0) {
		...
		while ($expected = xtc_db_fetch_array($expected_query,true)) {
			...
			if($gmSEOBoost->boost_products) {
				$gm_product_link = xtc_href_link($gmSEOBoost->get_boosted_product_url($expected['products_id'], $expected['products_name']) );
			} else {
				$gm_product_link = xtc_href_link(FILENAME_PRODUCT_INFO, xtc_product_link($expected['products_id'], $expected['products_name']));
			}

			$module_content[] = array ('PRODUCTS_LINK' => $gm_product_link, 'PRODUCTS_NAME' => $expected['products_name'], 'PRODUCTS_DATE' => xtc_date_short($expected['date_expected']));
