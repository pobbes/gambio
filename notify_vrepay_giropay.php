...
// write epay - LogFile
$vrepay->epay_log( "notify_vrepay_giropay getting POST´s -> " . print_r($_POST, TRUE) );

$statusNotify = strtoupper($_POST['STATUS']);
$orderId = $_POST['REFERENZNR'];
// orderId without prefix
$orderId = preg_replace('/'.substr(MODULE_PAYMENT_VREPAY_GIROPAY_ORDERPREFIX, 0, 5).'/', '', $orderId);
$amount = $_POST['BETRAG'];
$paytype = $_POST['ZAHLART'];
$rmsg = $_POST['RMSG'];

$vrepay->epay_log( "notify_vrepay_giropay orderId -> " . print_r($orderId, TRUE) );

// test, if md5 sum is correct
$md5_in = $_POST['ANTWGEHEIMNIS'];
$md5_new = md5($amount.$orderId.$paytype.MODULE_PAYMENT_VREPAY_GIROPAY_ANTWGEHEIMNIS);
...
if ($md5_in !== $md5_new) {
	...
	$vrepay->epay_log( "notify_vrepay_giropay sending result -> " . print_r($rc, TRUE) );
	echo $rc;
	return false;
}
...
switch ( $statusNotify ) {
	...
	case "RESERVIERT": 
		...
		$vrepay->epay_log( ("notify_vrepay_giropay regular payment status -> " . print_r('STATUS: ' . $statusNotify . ' - ' . $rmsg, TRUE)), true );	
	...
	default:
		...
		$vrepay->epay_log( ("notify_vrepay_giropay unknown status -> " . print_r('STATUS: ' . $statusNotify . ' - ' . $rmsg, TRUE)), true );
		break;
}
