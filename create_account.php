<?php
/* --------------------------------------------------------------
   create_account.php 2011-05-20 gm
   Gambio GmbH
   http://www.gambio.de
...
$breadcrumb->add(NAVBAR_TITLE_CREATE_ACCOUNT, xtc_href_link(FILENAME_CREATE_ACCOUNT, xtc_get_all_get_params(), 'SSL'));
...
// New VAT CHECK END
	...
	if (strlen($password) < ENTRY_PASSWORD_MIN_LENGTH) {
		$error = true;
		$smarty->assign('error_password', ENTRY_PASSWORD_ERROR);
		$messageStack->add('create_account', ENTRY_PASSWORD_ERROR);
	}
	elseif ($password != $confirmation) {
		$error = true;
		$smarty->assign('error_password2', ENTRY_PASSWORD_ERROR_NOT_MATCHING);
	...
	if ($error == false) {
		...
		$module_content = array ('MAIL_NAME' => htmlspecialchars_wrapper($name), 'MAIL_REPLY_ADDRESS' => EMAIL_SUPPORT_REPLY_ADDRESS, 'MAIL_GENDER' => htmlspecialchars_wrapper($gender));
      ...
      $campaign_check_query_raw = "SELECT *
		...
		if (ACTIVATE_GIFT_SYSTEM == 'true') {
			...
			if (NEW_SIGNUP_GIFT_VOUCHER_AMOUNT > 0) {
				...
				$insert_query = xtc_db_query("insert into ".TABLE_COUPON_EMAIL_TRACK." (coupon_id, customer_id_sent, sent_firstname, emailed_to, date_sent) values ('".$insert_id."', '0', 'Admin', '".xtc_db_input($email_address)."', now() )");
			...
			if (NEW_SIGNUP_DISCOUNT_COUPON != '') {
				...
				$insert_query = xtc_db_query("insert into ".TABLE_COUPON_EMAIL_TRACK." (coupon_id, customer_id_sent, sent_firstname, emailed_to, date_sent) values ('".$coupon_id."', '0', 'Admin', '".xtc_db_input($email_address)."', now() )");
		...
		{
			$t_gm_gift_link = xtc_href_link(FILENAME_GV_REDEEM, 'gv_no='.$coupon_code, 'NONSSL', false);
		...
		if(SEND_EMAILS == 'true') xtc_php_mail(EMAIL_SUPPORT_ADDRESS, EMAIL_SUPPORT_NAME, $email_address, $name, EMAIL_SUPPORT_FORWARDING_STRING, EMAIL_SUPPORT_REPLY_ADDRESS, EMAIL_SUPPORT_REPLY_ADDRESS_NAME, '', '', EMAIL_SUPPORT_SUBJECT, $html_mail, $txt_mail);
...
require (DIR_WS_INCLUDES.'header.php');
		...
		if(@isset($_SESSION['xtb0']['tx'][0]))
		{
			...
			if($process == false)
			{
				...
				$lstname = $GLOBALS['lastname'];
				$street_address = $GLOBALS['street_address'];
				$postcode = $GLOBALS['postcode'];
				$city = $GLOBALS['city'];
				$country = $GLOBALS['country'];
				$email_address = $GLOBALS['email_address'];
				$email_address_confirm = $GLOBALS['email_address'];
...
$smarty->assign('FORM_ACTION', xtc_draw_form('create_account', xtc_href_link(FILENAME_CREATE_ACCOUNT, $t_checkout_started_get_param, 'SSL'), 'post', '').xtc_draw_hidden_field('action', 'process'));
...
$smarty->assign('INPUT_PASSWORD', xtc_draw_password_fieldNote(array ('name' => 'password', 'text' => '&nbsp;'. (xtc_not_null(ENTRY_PASSWORD_TEXT) ? '<span class="inputRequirement">'.ENTRY_PASSWORD_TEXT.'</span>' : '')), '', '', 'gm_mb_input inactive_input'));
$smarty->assign('INPUT_PASSWORD_NAME', 'password');
$t_form_data_array['password']['name'] = 'password';
$t_form_data_array['password']['required'] = 1;
$t_form_data_array['password']['required_symbol'] = ENTRY_PASSWORD_TEXT;
$smarty->assign('INPUT_CONFIRMATION', xtc_draw_password_fieldNote(array ('name' => 'confirmation', 'text' => '&nbsp;'. (xtc_not_null(ENTRY_PASSWORD_CONFIRMATION_TEXT) ? '<span class="inputRequirement">'.ENTRY_PASSWORD_CONFIRMATION_TEXT.'</span>' : '')), '', '', 'gm_mb_input inactive_input'));
$smarty->assign('INPUT_CONFIRMATION_NAME', 'confirmation');
